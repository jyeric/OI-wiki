...
这时，枚举所有序列是不可行的。于是我们考虑将「判断一个序列是否合法」的过程（即内层 DP）抽象为一个 [确定有限状态自动机](../misc/fsm.md#%E7%A1%AE%E5%AE%9A%E6%80%A7%E6%9C%89%E9%99%90%E7%8A%B6%E6%80%81%E8%87%AA%E5%8A%A8%E6%9C%BA)（DFA）。一般地，对于固定的序列 $s\in\Sigma^n$，内层 DP 的状态函数可以表示为 $g(i,x;s)$，即已经处理完序列 $s$ 的长度为 $i$ 的前缀，且其他状态分量为 $x$ 时某个量的取值。相应地，内层 DP 的状态转移方程为
...
??? note "题解"
    ...
    因此，我们枚举 $T_{i+1}$，计算出 $x$ 转移后的状态 $x'$，再将 $f_{i+1,x'}$ 加上 $f_{i,x}$ 就可以完成外层 DP 的状态转移。最后，我们记录 $\textit{ans}*i$ 为 LCS 长度为 $i$ 的答案，枚举每个状态 $S$，$\textit{ans}*{\operatorname{popcount}(S)}$ 加上 $f_{m,S}$ 即可。
...
??? note "题解"
    ...
    $$
    \tilde G(g_{0/1,i-1}, x_i)*{j,k} = \max\left{(g*{0/1,i-1})_{\ell,j} + \ell + \left\lfloor\dfrac{x_i-\ell-j-k}{3}\right\rfloor:\ell+j+k\le x_i\right}.
    $$
...
- [CF979E Kuro and Topological Parity](https://codeforces.com/problemset/problem/979/E)<sup>[[存档](https://web.archive.org/web/20200117152122/https://codeforces.com/problemset/problem/979/E)]</sup>
