...
第一个问题相对容易解决。因为当直线斜率 $\lambda$ 发生变化时，所有这些直线切出的集合（即它们对应的上半平面的交）必然是一个凸集。因此，这些直线能够经过某个点，当且仅当这个点在该凸集的下凸壳上。这等价于说，函数 $v(y)$ 是 [凸函数](./slope-trick.md#%E7%A6%BB%E6%95%A3%E7%82%B9%E9%9B%86%E4%B8%8A%E7%9A%84%E5%87%B8%E5%87%BD%E6%95%B0)。
...
而对偶问题的目标函数是关于 $\lambda\in\mathbf R^d$ 的凹函数，从而是单峰函数，可以通过 [三分法](../../basic/binary.md#%E4%B8%89%E5%88%86%E6%B3%95) 或 [黄金分割法](../../basic/binary.md#%E4%BC%98%E5%8C%96%E9%BB%84%E9%87%91%E5%88%86%E5%89%B2%E6%B3%95) 高效地求解，复杂度仍然是 $O(T(n)\log^d L)$ 的。这就完全地解决了传统 WQS 二分方法中记录 $g(x_\lambda)$ 的值可能会出现的问题，同时，允许将 WQS 二分的思想应用至高维的情形。
...
接下来，本节将用凸分析的理论证明这些结论成立。至于这些方法的具体应用，可以参考 [例题](#%E4%BE%8B%E9%A2%98) 一节。


### Lagrange 对偶


考虑用 [Lagrange 乘子法](https://en.wikipedia.org/wiki/Lagrange_multiplier)<sup>[[存档](https://web.archive.org/web/20250907200944/https://en.wikipedia.org/wiki/Lagrange_multiplier)]</sup> 解决该问题。引入 Lagrange 乘子 $\lambda\in\mathbf R^d$，那么，Lagrangian 可以写作
...
交换两次最值操作，就得到它的 [对偶问题](<https://en.wikipedia.org/wiki/Duality_(optimization)>)<sup>[[存档](<https://web.archive.org/web/20231224000815/https://en.wikipedia.org/wiki/Duality_(optimization)>)]</sup>：
...
??? note "证明"
    ...
    第二步，利用 [超平面分离定理](https://en.wikipedia.org/wiki/Hyperplane_separation_theorem)<sup>[[存档](https://web.archive.org/web/20241222174903/https://en.wikipedia.org/wiki/Hyperplane_separation_theorem)]</sup> 说明 $f(x)\le f^{**}(x)$。假设不然，存在 $x_0\in\mathbf R^d$ 使得 $f(x_0)>f^{**}(x_0)$ 成立。因为 $f(x)$ 的上境图 $\operatorname{epi}(f)$ 是闭凸集，而且单点集 ${(x_0,f^{**}(x_0))}$ 是紧凸集，所以，根据超平面分离定理，存在 $(\lambda,t)\in\mathbf R^d\times\mathbf R$ 和 $\alpha\in\mathbf R$ 使得对于所有 $x\in\operatorname{dom} f:={x\in\mathbf R^d:f(x)<+\infty}$ 和所有 $y\ge f(x)$ 都有
...
上一节说明了，带惩罚的问题的价值函数 $h(\lambda)$ 是原问题的价值函数 $v(y)$ 的凸共轭的相反数。因为凸共轭的定义实际上是一个含参数的最优化问题，所以它也成立类似 [包络定理](https://en.wikipedia.org/wiki/Envelope_theorem)<sup>[[存档](https://web.archive.org/web/20231207102152/https://en.wikipedia.org/wiki/Envelope_theorem)]</sup> 的结论。但是，因为凸函数并非处处可微的，所以需要首先将导数的定义推广到凸函数的情形。这就引出了次梯度的概念。
...
- 利用状态转移方程也可以归纳地证明凸性成立，过程中可能会用到一些 [保持凸性的变换](./slope-trick.md#%E5%87%B8%E5%87%BD%E6%95%B0%E7%9A%84%E5%8F%98%E6%8D%A2)；
...
??? example "凸性证明一"
    ...
    利用该费用流模型，可以通过模拟费用流或 [反悔贪心](../../basic/greedy.md#%E5%90%8E%E6%82%94%E8%A7%A3%E6%B3%95) 的方法在 $O(n\log n)$ 的复杂度内解决该问题。
...
算法竞赛中，另一类常见的成立凸性的问题是 [区间分拆问题](./quadrangle.md#%E5%8C%BA%E9%97%B4%E5%88%86%E6%8B%86%E9%97%AE%E9%A2%98)。该页面证明了，如果单个区间的成本函数满足四边形不等式，那么限制区间个数的区间分拆问题的最小成本是区间个数的凸函数。该页面同样提供了一些判断某个函数 $w(l,r)$ 是否满足四边形不等式的方法。最为直接的方法就是计算它的二阶混合差分：
...
???+ warning "「边际成本递增」并不一定导致凸性"
    ...
    组合优化问题中，目标函数常常具有一些「边际成本递增」的性质，但是这并不必然导致凸性。一个典型的例子是 [[IOI 2005] Riv 河流](https://www.luogu.com.cn/problem/P3354)<sup>[[存档](https://web.archive.org/web/20210511112347/https://www.luogu.com.cn/problem/P3354)]</sup>，该问题的链上版本是满足四边形不等式因而具有凸性的，但是树上版本存在凸性不成立的例子。
...
??? note "解答"
    ...
    建立了函数 $v(m)$ 的凸性后，就可以用 WQS 二分解决该问题。移除数量限制并将每条白边的权重都减去 $k$，并求解最小生成树问题。为此，可以应用 [Kruskal 算法](../../graph/mst.md#kruskal-%E7%AE%97%E6%B3%95)。利用并查集维护连通性，算法的复杂度就是 $O(E\log E+E\alpha(V))$，其中，$E$ 和 $V$ 分别为边数和顶点数，$\alpha(\cdot)$ 为反 Ackerman 函数。复杂度的主要部分 $O(E\log E)$ 是给边排序的复杂度，在本题中可以进一步优化。虽然在 WQS 二分的过程中需要多次计算最小生成树，但是每次只有白边的边权会整体加减一个数。所以，可以在预处理时给白边、黑边分别排序，然后每次计算最小生成树时，只需要将调整完权重后的白边和黑边归并到一起就可以了。这样，整体复杂度就降低到了 $O(E\log E+E\alpha(V)\log L)$，其中，$L$ 为边权的取值范围的长度。
...
??? note "解答"
    ...
    这是典型的 [区间分拆问题](./quadrangle.md#%E5%8C%BA%E9%97%B4%E5%88%86%E6%8B%86%E9%97%AE%E9%A2%98)。二分队列的实现细节请参考该页面。
...
??? note "解答"
    ...
    这是若干个凸函数的 [卷积下确界](./slope-trick.md#%E5%8D%B7%E7%A7%AF%E4%B8%8B%E7%A1%AE%E7%95%8Cminkowski-%E5%92%8C)，所以也是凸函数。如果题目要求的是 $v(m)$，那么，可以使用与之前的例题一致的方法求解，时间复杂度为 $O(n\log^2L)$；但是，本题求的是满足 $v(m)\le V$ 的最小的 $m$。利用 WQS 二分计算 $v(m)$ 再对 $m$ 二分的方法是行不通的，它的复杂度达到了 $O(n\log^3L)$。就本题而言，有如下两种处理方法。
...
- [Luogu P1484 种树](https://www.luogu.com.cn/problem/P1484)<sup>[[存档](https://web.archive.org/web/20221203194704/https://www.luogu.com.cn/problem/P1484)]</sup>
- [Luogu P1792 [国家集训队] 种树](https://www.luogu.com.cn/problem/P1792)<sup>[[存档](https://web.archive.org/web/20241212042338/https://www.luogu.com.cn/problem/P1792)]</sup>
- [Luogu P2619 [国家集训队] Tree I](https://www.luogu.com.cn/problem/P2619)<sup>[[存档](https://web.archive.org/web/20241008082138/https://www.luogu.com.cn/problem/P2619)]</sup>
- [Luogu P3620 [APIO/CTSC2007] 数据备份](https://www.luogu.com.cn/problem/P3620)<sup>[[存档](https://web.archive.org/web/20250809182909/https://www.luogu.com.cn/problem/P3620)]</sup>
- [Luogu P4072 [SDOI2016] 征途](https://www.luogu.com.cn/problem/P4072)<sup>[[存档](https://web.archive.org/web/20221130100130/https://www.luogu.com.cn/problem/P4072)]</sup>
- [Luogu P4383 [八省联考 2018] 林克卡特树](https://www.luogu.com.cn/problem/P4383)<sup>[[存档](https://web.archive.org/web/20250815180605/https://www.luogu.com.cn/problem/P4383)]</sup>
- [Luogu P4983 忘情](https://www.luogu.com.cn/problem/P4983)<sup>[[存档](https://web.archive.org/web/20240925105039/https://www.luogu.com.cn/problem/P4983)]</sup>
- [Luogu P5308 [COCI 2018/2019 #4] Akvizna](https://www.luogu.com.cn/problem/P5308)<sup>[[存档](https://web.archive.org/web/20200925110722/https://www.luogu.com.cn/problem/P5308)]</sup>
- [Luogu P5633 最小度限制生成树](https://www.luogu.com.cn/problem/P5633)<sup>[[存档](https://web.archive.org/web/20241007164109/https://www.luogu.com.cn/problem/P5633)]</sup>
- [Luogu P5896 [IOI 2016] aliens](https://www.luogu.com.cn/problem/P5896)<sup>[[存档](https://web.archive.org/web/20250413153255/https://www.luogu.com.cn/problem/P5896)]</sup>
- [Luogu P6246 [IOI 2000] 邮局 加强版 加强版](https://www.luogu.com.cn/problem/P6246)<sup>[[存档](https://web.archive.org/web/20231014085413/https://www.luogu.com.cn/problem/P6246)]</sup>
- [AtCoder Beginner Contest 218 H - Red and Blue Lamps](https://atcoder.jp/contests/abc218/tasks/abc218_h)<sup>[[存档](https://web.archive.org/web/20241116144909/https://atcoder.jp/contests/abc218/tasks/abc218_h)]</sup>
- [AtCoder Beginner Contest 305 Ex - Shojin](https://atcoder.jp/contests/abc305/tasks/abc305_h)<sup>[[存档](https://web.archive.org/web/20241005014730/https://atcoder.jp/contests/abc305/tasks/abc305_h)]</sup>
- [AtCoder Regular Contest 164 E - Segment-Tree Optimization](https://atcoder.jp/contests/arc164/tasks/arc164_e)<sup>[[存档](https://web.archive.org/web/20240719032816/https://atcoder.jp/contests/arc164/tasks/arc164_e)]</sup>
- [Codeforces 125 E. MST Company](https://codeforces.com/problemset/problem/125/E)<sup>[[存档](https://web.archive.org/web/20200813232215/https://codeforces.com/problemset/problem/125/E)]</sup>
- [Codeforces 321 E. Ciel and Gondolas](https://codeforces.com/problemset/problem/321/E)<sup>[[存档](https://web.archive.org/web/20211020225319/https://codeforces.com/problemset/problem/321/E)]</sup>
- [Codeforces 739 E. Gosha is hunting](https://codeforces.com/problemset/problem/739/E)<sup>[[存档](https://web.archive.org/web/20210926032833/https://codeforces.com/problemset/problem/739/E)]</sup>
- [Codeforces 802 O. April Fools' Problem (hard)](https://codeforces.com/contest/802/problem/O)<sup>[[存档](https://web.archive.org/web/20170617122853/https://codeforces.com/contest/802/problem/O)]</sup>
- [Codeforces 958 E2. Guard Duty (medium)](https://codeforces.com/problemset/problem/958/E2)<sup>[[存档](https://web.archive.org/web/20181225163609/https://codeforces.com/problemset/problem/958/E2)]</sup>
- [Codeforces 1279 F. New Year and Handle Change](https://codeforces.com/problemset/problem/1279/F)<sup>[[存档](https://web.archive.org/web/20210727131156/https://codeforces.com/problemset/problem/1279/F)]</sup>
- [Codeforces 1661 F. Teleporters](https://codeforces.com/problemset/problem/1661/F)<sup>[[存档](https://web.archive.org/web/20220505082524/https://codeforces.com/problemset/problem/1661/F)]</sup>
- [Codeforces 1799 F. Halve or Subtract](https://codeforces.com/problemset/problem/1799/F)<sup>[[存档](https://web.archive.org/web/20230403051318/https://codeforces.com/problemset/problem/1799/F)]</sup>
- [2019 Summer Petrozavodsk Camp H. Honorable Mention](https://codeforces.com/gym/102331/problem/H)<sup>[[存档](https://web.archive.org/web/20240912182241/https://codeforces.com/gym/102331/problem/H)]</sup>

## 参考资料与注释


- [王钦石《浅析一类二分方法》](https://www.doc88.com/p-949564862405.html)<sup>[[存档](https://web.archive.org/web/20181211120954/https://www.doc88.com/p-949564862405.html)]</sup>
- [Theoretical grounds of lambda optimization by adamant - Codeforces blog](https://codeforces.com/blog/entry/98334)<sup>[[存档](https://web.archive.org/web/20250906082334/https://codeforces.com/blog/entry/98334)]</sup>
- [严谨的 WQS 二分方法 by YeahPotato - 洛谷博客](https://www.luogu.com.cn/article/vsffwrc3)<sup>[[存档](https://web.archive.org/web/20250813134130/https://www.luogu.com.cn/article/vsffwrc3)]</sup>
- [【学习笔记】WQS 二分详解及常见理解误区解释 by ikrvxt - CSDN 博客](https://blog.csdn.net/Emm_Titan/article/details/124035796)<sup>[[存档](https://web.archive.org/web/20250714091507/https://blog.csdn.net/Emm_Titan/article/details/124035796)]</sup>
- [Convex conjugate - Wikipedia](https://en.wikipedia.org/wiki/Convex_conjugate)<sup>[[存档](https://web.archive.org/web/20241211061043/https://en.wikipedia.org/wiki/Convex_conjugate)]</sup>
- [Fenchel–Moreau theorem - Wikipedia](https://en.wikipedia.org/wiki/Fenchel%E2%80%93Moreau_theorem)<sup>[[存档](https://web.archive.org/web/20250614193842/https://en.wikipedia.org/wiki/Fenchel%E2%80%93Moreau_theorem)]</sup>
- [Subderivative - Wikipedia](https://en.wikipedia.org/wiki/Subderivative)<sup>[[存档](https://web.archive.org/web/20241229013528/https://en.wikipedia.org/wiki/Subderivative)]</sup>
- [Boyd, Stephen P., and Lieven Vandenberghe. Convex optimization. Cambridge university press, 2004.](https://web.stanford.edu/~boyd/cvxbook/bv_cvxbook.pdf)<sup>[[存档](https://web.archive.org/web/20231231111013/https://web.stanford.edu/~boyd/cvxbook/bv_cvxbook.pdf)]</sup>
...
[^high-d-convex]: 实际问题中，$y$ 可能只能取到 $\mathbf R^d$ 中的有限多个格点。此处实际需要的条件是，原问题的解 $v(y)$ 可以延拓为 $\mathbf R^d$ 上的凸函数 $\tilde v:\mathbf R^d\rightarrow \mathbf R\cup{\pm\infty}$，也就是说 $v(y)$ 是 **可凸延拓的**（convex-extensible）。为行文方便，正文中仍然用 $v(y)$ 表示延拓后的函数。几何直观上，这相当于说点集 ${(y,v(y))}$ 全部都位于它们的凸包的下凸壳上。对于一维的情形，这一条件利用代数语言 [很容易刻画](./slope-trick.md#%E7%A6%BB%E6%95%A3%E7%82%B9%E9%9B%86%E4%B8%8A%E7%9A%84%E5%87%B8%E5%87%BD%E6%95%B0)；但是，对于高维的情形，这稍微有些复杂，[这份讲义](https://kzmurota.fpark.tmu.ac.jp/paper/HIMSummerSchool15Murota.pdf)<sup>[[存档](https://web.archive.org/web/20231130095858/https://kzmurota.fpark.tmu.ac.jp/paper/HIMSummerSchool15Murota.pdf)]</sup> 中提供了一些简单的充分条件。
...
[^mst]: 最小生成树问题有两种常见的写成线性规划问题的 [方法](https://math.arizona.edu/~glickenstein/math443f14/golari.pdf)<sup>[[存档](https://web.archive.org/web/20190214190518/https://math.arizona.edu/~glickenstein/math443f14/golari.pdf)]</sup>：子回路消除模型（subtour-elimination formulation）和基于割集的模型（cut-based formulation）。只有前一种建模方式能够保证得到的线性规划问题和原问题是等价的。
[^edge-swap]: 这一引理对于一般的 [拟阵](../../math/matroid.md) 也是成立的。它称为 **对称基交换性质**（symmetric base-exchange property），相关资料可以参考 [Wikipedia 页面](https://en.wikipedia.org/wiki/Basis_of_a_matroid)<sup>[[存档](https://web.archive.org/web/20221101002011/https://en.wikipedia.org/wiki/Basis_of_a_matroid)]</sup>。因此，本题关于凸性的结论可以推广到一般的拟阵上。
