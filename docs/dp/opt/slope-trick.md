...
几何直观上，$\operatorname{epi}h$ 就是 $\operatorname{epi}f$ 和 $\operatorname{epi}g$ 的 [Minkowski 和](../../geometry/convex-hull.md#%E9%97%B5%E5%8F%AF%E5%A4%AB%E6%96%AF%E5%9F%BA%E5%92%8C)。如果 $f$ 和 $g$ 都是分段线性函数，那么 $h$ 同样是分段线性函数，且它的斜率段可以看作是 $f$ 和 $g$ 的斜率段合并（再排序）的结果。
...
??? note "解答"
    ...
    2. 对得到的函数取最小值，将 $g(x)=f_{i-1}(x)+|a'*i-x|$ 变为 $f_i(x)=\min*{y\le x}g(y)$。根据前文分析，这相当于对 $g(x)$ 和 $0_{[0,+\infty)}$ 做卷积下确界。因为后者的斜率段只有一段，斜率为 $0$ 且向右延伸至无限长，将其插入 $g(x)$ 的斜率段中，相当于删除其中所有正斜率段。
    ...
    $$
    f_{i-1}(\max{a'*i,\xi*{-1}})+|\max{a'*i,\xi*{-1}}-a'*i|=f*{i-1}(\xi_{-1})+\max{0,\xi_{-1}-a'_i}.
    $$
    ...
    本题还要求输出一种最优方案。因为最后操作结束时，最优解就是堆顶，所以 $b'*n$ 的取值可以直接确定。如果已经知道了第 $i$ 个最优解 $b'_i$，要求解 $f*{i-1}(x)$ 满足 $x\le b'$ 的最优解，只需要注意到因为 $f_{i-1}(x)$ 是凸的，所以越接近它的全局最小值点，解就越优，故而只要记录 $f_{i-1}(x)$ 的全局最小值点，并将它与 $b'*i$ 取最小值，就可以得到最优的 $b'*{i-1}$。
...
- [Codeforces 713 C. Sonya and Problem Without a Legend](https://codeforces.com/problemset/problem/713/C)<sup>[[存档](https://web.archive.org/web/20240916204003/https://codeforces.com/problemset/problem/713/C)]</sup>
- [Luogu P2893 [USACO08FEB] Making the Grade G](https://www.luogu.com.cn/problem/P2893)<sup>[[存档](https://web.archive.org/web/20250815191052/https://www.luogu.com.cn/problem/P2893)]</sup>
- [Luogu P4331 [BalticOI 2004] Sequence 数字序列](https://www.luogu.com.cn/problem/P4331)<sup>[[存档](https://web.archive.org/web/20250815193745/https://www.luogu.com.cn/problem/P4331)]</sup>
- [Luogu P4597 序列 sequence](https://www.luogu.com.cn/problem/P4597)<sup>[[存档](https://web.archive.org/web/20250717125958/https://www.luogu.com.cn/problem/P4597)]</sup>
- [AtCoder 第 2 回 ドワンゴからの挑戦状 予選 E - 花火](https://atcoder.jp/contests/dwango2016-prelims/tasks/dwango2016qual_e)<sup>[[存档](https://web.archive.org/web/20250719140259/https://atcoder.jp/contests/dwango2016-prelims/tasks/dwango2016qual_e)]</sup>
...
??? note "解答"
    ...
    显然，对于本题，将正负斜率段分别维护较为方便。因为操作主要集中在零斜率段附近，因此考虑使用 [对顶堆](../../ds/binary-heap.md#%E5%AF%B9%E9%A1%B6%E5%A0%86)，即分别用最大堆和最小堆维护负斜率段和正斜率段的拐点。拐点的整体平移操作用懒标记完成。因为第二步操作需要分别对两个堆插入一个 $a_i$，而且，插入完成后，未必最大堆的堆顶仍然小于等于最小堆的堆顶。此时，交换两堆顶，直到堆顶的大小关系得到满足即可。
...
- [Luogu P4272 [CTSC2009] 序列变换](https://www.luogu.com.cn/problem/P4272)<sup>[[存档](https://web.archive.org/web/20250713140147/https://www.luogu.com.cn/problem/P4272)]</sup>
- [Luogu P11598 [NOISG 2018 Finals] Safety](https://www.luogu.com.cn/problem/P11598)<sup>[[存档](https://web.archive.org/web/20250713144828/https://www.luogu.com.cn/problem/P11598)]</sup>
- [AtCoder Beginner Contest 217 H - Snuketoon](https://atcoder.jp/contests/abc217/tasks/abc217_h)<sup>[[存档](https://web.archive.org/web/20220711043549/https://atcoder.jp/contests/abc217/tasks/abc217_h)]</sup>
- [AtCoder Regular Contest 070 E - NarrowRectangles](https://atcoder.jp/contests/arc070/tasks/arc070_c)<sup>[[存档](https://web.archive.org/web/20240809185249/https://atcoder.jp/contests/arc070/tasks/arc070_c)]</sup>
- [AtCoder Regular Contest 123 D - Inc, Dec - Decomposition](https://atcoder.jp/contests/arc123/tasks/arc123_d)<sup>[[存档](https://web.archive.org/web/20240623053944/https://atcoder.jp/contests/arc123/tasks/arc123_d)]</sup>

## 维护斜率


还有一些问题，维护斜率更为方便。这类问题通常也可以使用 [反悔贪心](../../basic/greedy.md#%E5%90%8E%E6%82%94%E8%A7%A3%E6%B3%95) 或模拟费用流的思想解决。费用流模型中，最小费用往往是流量的凸函数，这就为使用 Slope Trick 提供了基础。
...
??? note "解答"
    ...
    对比该算法实现与上文 [最小成本递增序列](#%E4%BE%8B%E9%A2%98%E6%9C%80%E5%B0%8F%E6%88%90%E6%9C%AC%E9%80%92%E5%A2%9E%E5%BA%8F%E5%88%97) 的代码可知，该算法等价于求将股票价格变为弱递减序列的最小成本。
...
- [Codeforces 865 D. Buy Low Sell High](https://codeforces.com/problemset/problem/865/D)<sup>[[存档](https://web.archive.org/web/20171219030156/https://codeforces.com/problemset/problem/865/D)]</sup>
...
- [Luogu P2748 [USACO16OPEN] Landscaping P](https://www.luogu.com.cn/problem/P2748)<sup>[[存档](https://web.archive.org/web/20231006080857/https://www.luogu.com.cn/problem/P2748)]</sup>
- [Kyoto University PC 2016 H - WAAAAAAAAAAAAALL](https://atcoder.jp/contests/kupc2016/tasks/kupc2016_h)<sup>[[存档](https://web.archive.org/web/20250813033625/https://atcoder.jp/contests/kupc2016/tasks/kupc2016_h)]</sup>
- [JAG Practice Contest 2017 J - Farm Village](https://atcoder.jp/contests/jag2017autumn/tasks/jag2017autumn_j)<sup>[[存档](https://web.archive.org/web/20221207162354/https://atcoder.jp/contests/jag2017autumn/tasks/jag2017autumn_j)]</sup>
...
- [Luogu P3642 [APIO2016] 烟火表演](https://www.luogu.com.cn/problem/P3642)<sup>[[存档](https://web.archive.org/web/20250620010038/https://www.luogu.com.cn/problem/P3642)]</sup>
- [Luogu P9962 [THUPC 2024 初赛] 一棵树](https://www.luogu.com.cn/problem/P9962)<sup>[[存档](https://web.archive.org/web/20250713141514/https://www.luogu.com.cn/problem/P9962)]</sup>
- [Luogu P11317 [RMI 2021] 路径/Paths](https://www.luogu.com.cn/problem/P11317)<sup>[[存档](https://web.archive.org/web/20250713145016/https://www.luogu.com.cn/problem/P11317)]</sup>
- [AtCoder Beginner Contest 383 G - Bar Cover](https://atcoder.jp/contests/abc383/tasks/abc383_g)<sup>[[存档](https://web.archive.org/web/20250810204423/https://atcoder.jp/contests/abc383/tasks/abc383_g)]</sup>
- [Codeforces 280 D. k-Maximum Subsequence Sum](https://codeforces.com/problemset/problem/280/D)<sup>[[存档](https://web.archive.org/web/20250820082738/https://codeforces.com/problemset/problem/280/D)]</sup>
- [Codeforces 280 E. Sequence Transformation](https://codeforces.com/problemset/problem/280/E)<sup>[[存档](https://web.archive.org/web/20230607181321/https://codeforces.com/problemset/problem/280/E)]</sup>
...
- [Codeforces 1209 H. Moving Walkways](https://codeforces.com/contest/1209/problem/H)<sup>[[存档](https://web.archive.org/web/20221221113248/https://codeforces.com/contest/1209/problem/H)]</sup>
- [Codeforces 1229 F. Mateusz and Escape Room](https://codeforces.com/contest/1229/problem/F)<sup>[[存档](https://web.archive.org/web/20221221151421/https://codeforces.com/contest/1229/problem/F)]</sup>
- [Codeforces 1534 G. A New Beginning](https://codeforces.com/problemset/problem/1534/G)<sup>[[存档](https://web.archive.org/web/20210802143942/https://codeforces.com/problemset/problem/1534/G)]</sup>
- [Codeforces 1787 H. Codeforces Scoreboard](https://codeforces.com/problemset/problem/1787/H)<sup>[[存档](https://web.archive.org/web/20240418115813/https://codeforces.com/problemset/problem/1787/H)]</sup>
...
- [2018 ACM-ICPC World Finals C. Conquer The World](https://codeforces.com/gym/102482/problem/C)<sup>[[存档](https://web.archive.org/web/20240823204006/https://codeforces.com/gym/102482/problem/C)]</sup>
- [300iq Contest 3 F. Farm of Monsters](https://codeforces.com/gym/102538/problem/F)<sup>[[存档](https://web.archive.org/web/20220123115242/https://codeforces.com/gym/102538/problem/F)]</sup>

## 参考文献与注释


- [[Tutorial] Slope Trick - zscoder](https://codeforces.com/blog/entry/47821)<sup>[[存档](https://web.archive.org/web/20210507031513/https://codeforces.com/blog/entry/47821)]</sup>
- [Slope trick explained - Kuroni](https://codeforces.com/blog/entry/77298)<sup>[[存档](https://web.archive.org/web/20201109094620/https://codeforces.com/blog/entry/77298)]</sup>
- [Slope Trick - USACO Guide](https://usaco.guide/adv/slope-trick?lang=cpp)<sup>[[存档](https://web.archive.org/web/20241222154035/https://usaco.guide/adv/slope-trick?lang=cpp)]</sup>
- [[Tutorial] Intuition on Slope Trick - maomao90](https://codeforces.com/blog/entry/103222)<sup>[[存档](https://web.archive.org/web/20220818194353/https://codeforces.com/blog/entry/103222)]</sup>
