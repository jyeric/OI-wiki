...
如果您已迫不及待，想要快速上手，建议先阅读 [太长不看版](#%E5%A4%AA%E9%95%BF%E4%B8%8D%E7%9C%8B%E7%89%88) 与 [图片举例](#%E5%9B%BE%E8%A7%A3) 的章节。
...
- FREQ-2：除 [太长不看版](#%E5%A4%AA%E9%95%BF%E4%B8%8D%E7%9C%8B%E7%89%88) 一节外，格式手册的条目都需要有不重复的编号，编号需要匹配正则表达式 `(?<category>[A-Z]{4})-(?<id>[1-9][0-9]*(?:\.[1-9][0-9]*)*)`，其中 `category` 应具有直观的含义。说明文字不需要有编号。
- FREQ-3：[太长不看版](#%E5%A4%AA%E9%95%BF%E4%B8%8D%E7%9C%8B%E7%89%88) 的条目必须来自格式手册其他章节的内容，且需在末尾引用对应的条目编号。
...
- SAVE-3：推荐使用 SVG 格式的图片[^ref4]，以获取较好的清晰度和缩放效果。由于 **OI Wiki** 各组件对 SVG 标准的兼容性不同，所以您的图片应基于 [SVG 1.1](http://www.w3.org/TR/SVG11/) 标准。

- SAVE-4：动图如果无法或者不会制作 SVG 格式的，则推荐使用 APNG 格式[^apng]的文件。Windows 用户可使用 [ScreenToGif](https://www.screentogif.com)<sup>[[存档](https://web.archive.org/web/20171231131534/https://www.screentogif.com)]</sup> 录制，Linux 用户可使用 [Peek](https://github.com/phw/peek)<sup>[[存档](https://web.archive.org/web/20250909163016/https://github.com/phw/peek)]</sup> 录制，注意需要在设置里调整为录制 APNG。其他情况则推荐先制作为 MP4 等视频文件再转换为 APNG，如果使用 ffmpeg 则可以使用 `ffmpeg -i filename.mp4 -f apng filename.apng -plays 0` 转换。[^intro-apng]
...
- SAVE-7：站内链接请去掉网站域名，并且使用相对路径链接对应 `.md` 文件。例如，在本页面（`intro/format`）中链接杂项简介（`misc`），应使用 `[杂项简介](../misc/index.md)`。可以在链接中添加 hash 来链接到某一节，例如 [`[Pull Request 信息格式规范](./htc.md#pull-request-信息格式规范)`](./htc.md#pull-request-%E4%BF%A1%E6%81%AF%E6%A0%BC%E5%BC%8F%E8%A7%84%E8%8C%83)，hash 的值可以通过位于每个标题右侧的按钮或者位于网页右侧的目录中的链接得到。
...
除现有内容质量较低的情况外，建议尽量从 **补充** 的角度来做贡献，而非采取直接覆盖的方式。如果拿不准主意，可以参考 [关于本项目的交流方式](./about.md#%E4%BA%A4%E6%B5%81%E6%96%B9%E5%BC%8F) 一节，与 **OI Wiki** 项目组联系。
...
[remark-lint](https://github.com/remarkjs/remark-lint)<sup>[[存档](https://web.archive.org/web/20241231195203/https://github.com/remarkjs/remark-lint)]</sup> 可以自动给项目内文件统一风格。**OI Wiki** 现在启用的配置文件托管在 [.remarkrc](https://github.com/OI-wiki/OI-wiki/blob/master/.remarkrc)<sup>[[存档](https://web.archive.org/web/20230608065137/https://github.com/OI-wiki/OI-wiki/blob/master/.remarkrc)]</sup>。
...
- LINT-3：由于 remark-lint 不能很好地处理删除线，因此请不要使用删除线语法（不使用删除线语法的另外一个原因是，删除线划去的内容大多为「抖机灵」性质，对读者理解帮助不大，不符合下面的「文本内容的格式要求」中对内容表述的要求）。
...
- PUNC-2：请正确使用 **全角** 标点符号与 **半角** 标点符号。汉语请使用全角符号，英语请使用半角符号。中文中夹用英文时，请参考 [中文出版物夹用英文的编辑规范](https://www.nppa.gov.cn/xxgk/fdzdgknr/hybz/202210/t20221004_445147.html)<sup>[[存档](https://web.archive.org/web/20250413090857/https://www.nppa.gov.cn/xxgk/fdzdgknr/hybz/202210/t20221004_445147.html)]</sup>。

- PUNC-3：由于 `“……”` 未区分全半角，请使用 `「……」` 作为全角引号，`"..."` 作为半角引号。
...
- PUNC-8：注意区分各种不同的连接号，如 hyphen（一般使用 U+002D hyphen-minus（-），即键盘上的「减号」代替），U+2013 en dash（–）和 U+2014 em dash（—）。（英文中连接多个人名时，须用 en dash，但是极常误用为 hyphen。其他误用较为罕见，基本上只需记住这一点即可。）详见 [连接号 - 维基百科](https://zh.wikipedia.org/wiki/%E8%BF%9E%E6%8E%A5%E5%8F%B7)<sup>[[存档](https://web.archive.org/web/20241117204419/https://zh.wikipedia.org/wiki/%E8%BF%9E%E6%8E%A5%E5%8F%B7)]</sup>。
...
- MDFM-5：建议使用主题扩展的 `???+note` 格式（即 [Collapsible Blocks](https://squidfunk.github.io/mkdocs-material/reference/admonitions/#collapsible-blocks)<sup>[[存档](https://web.archive.org/web/20250907050702/https://squidfunk.github.io/mkdocs-material/reference/admonitions/#collapsible-blocks)]</sup>）来描述题面和参考代码。也可以用这种格式来展示其他需要补充介绍的内容。
  ...
  ???+ note "[HDOJ 的「A + B Problem」](https://acm.hdu.edu.cn/showproblem.php?pid=1000)<sup>[[存档](https://web.archive.org/web/20111223001329/https://acm.hdu.edu.cn/showproblem.php?pid=1000)]</sup>"
  标题也可以使用 Markdown 的超链接。这里的超链接是 HDOJ 的「A + B Problem」。
  ...
  折叠框的标题，即 `???+note` 中 `note` 后的内容应以 `"` 包裹起来。其中的内容支持 Markdown 语法。详见 [Admonition - Changing the title](https://squidfunk.github.io/mkdocs-material/reference/admonitions/#changing-the-title)<sup>[[存档](https://web.archive.org/web/20211231150140/https://squidfunk.github.io/mkdocs-material/reference/admonitions/#changing-the-title)]</sup>。（不具备折叠功能的为一般的 Admonitions，参考 [Admonitions - Material for MkDocs](https://squidfunk.github.io/mkdocs-material/reference/admonitions)<sup>[[存档](https://web.archive.org/web/20211231150140/https://squidfunk.github.io/mkdocs-material/reference/admonitions)]</sup>）

- MDFM-6：当需要添加不同语言的代码时，推荐使用 Content tabs，可以实现不同语言代码的切换。Content tabs 还有其他的用法，详见 [Content tabs](https://squidfunk.github.io/mkdocs-material/reference/content-tabs/#usage)<sup>[[存档](https://web.archive.org/web/20201026105046/https://squidfunk.github.io/mkdocs-material/reference/content-tabs/#usage)]</sup>。其使用方法和效果如下。
...
如果对 mkdocs-material（我们使用的这个主题）还有什么问题，还可以查阅 [MkDocs 使用说明](https://github.com/ctf-wiki/ctf-wiki/wiki/Mkdocs-%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E)<sup>[[存档](https://web.archive.org/web/20220818005418/https://github.com/ctf-wiki/ctf-wiki/wiki/Mkdocs-%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E)]</sup>，其介绍了 mkdocs-material 主题的插件使用方式。
...
- CONT-6：请尽量为链接提供完整的标题、或者可被识别的提示，避免使用裸地址和「这」、「此」之类的模糊不清的描述。每一个超链接都应尽量对其加以清楚明确的描述，方便读者明白该超链接将指向何处。
  ...
  ???+ failure "不推荐的写法"
  ```markdown
  请参考[这个页面](https://docs.github.com/en/github/collaborating-with-issues-and-pull-requests/syncing-a-fork)<sup>[[存档](https://web.archive.org/web/20241231195142/https://docs.github.com/en/github/collaborating-with-issues-and-pull-requests/syncing-a-fork)]</sup>
...
- CONT-10：请注意代码的可读性。
  ...
  - CONT-10.2.3：在声明 [类型别名](https://en.cppreference.com/w/cpp/language/type_alias)<sup>[[存档](https://web.archive.org/web/20181226151001/https://en.cppreference.com/w/cpp/language/type_alias)]</sup> 时，不推荐使用 `typedef`，推荐使用 `using`。

  - CONT-10.2.4：不推荐用宏定义定义常量，推荐直接使用 `constexpr`/`const` 等关键字定义常量。

  - CONT-10.2.5：不推荐对函数使用 `inline` 关键字，详见 [编译优化](../lang/optimizations.md#inline---%E5%86%85%E8%81%94)。
...
- MATH-1.1：您使用的符号不应与 [数学符号表](./symbol.md) 规定的符号冲突。
...
- MATH-1.2：使用 Roman 体表示数字、常量、算子和函数。使用 Italic 体表示变量、下标。LaTeX 已经预先定义好了一些常见的常量、函数、运算符等，我们可以直接调用，包括但不限于：
  ...
  所以在输入常量、函数名、运算符等时，请先检查一下是否应该使用 Roman 体或其它字体。LaTeX 符号的书写可参考 [KaTeX 的 Supported Functions 页面](https://katex.org/docs/supported.html)<sup>[[存档](https://web.archive.org/web/20250909164142/https://katex.org/docs/supported.html)]</sup>（不是全部），也可以搜索求解。
...
- MATH-1.5：组合数请使用 `\dbinom{n}{m}`，效果 $\dbinom{n}{m}$，而不是 `{n \choose m}`（在 LaTeX 中这种写法已不推荐）；与上一条关于分数的约定相似，请不要使用 `\binom{n}{m}`，效果 $\binom{n}{m}$。

- MATH-1.6：尽可能避免在行内使用巨运算符（如 $\sum$，$\prod$，$\int$ 等）。

- MATH-1.7：在不会引起歧义的情况下，请用 `$\times$` 代替星号，叉乘请使用 `$\times$`，点乘请使用 `$\cdot$`。如 $a\times b$，$a\cdot b$，而不是 $a\ast b$。

- MATH-1.8：请用 `$\cdots$`（居于排版基线与顶线中间），`$\ldots$`（居于排版基线的位置），`$\vdots$`（竖着的省略号）代替 `$...$`。如 $a_1,a_2,\cdots a_n$，而不是 $a_1,a_2,... a_n$。

- MATH-1.9：请注意，不要在非代码区域使用任何程序设计语言的表示方式，而是使用 LaTeX 公式。例如，使用 `$=$` 而不是 `$==$`（如 $a=b$，而不是 $a==b$）、使用 ``a<<1`` 或者 `$a\times 2$` 而不是 `$a<<1$`、使用 `$a\bmod b$` 代替 `$a\%b$`（如 $a\bmod b$，而不是 $a%b$）等。

- MATH-1.10：公式中不要使用中括号连缀（即 C++ 高维数组的表示方式）而多使用下标。即 $a_{i,j,k}$ 而不是 $a[i][j][k]$。在公式中下标较复杂的情况下建议改用多元函数（$f(i,j,k)$）或内联代码格式。对于一元简单函数使用 `$f_i$`、`$f(i)$` 或 `$f[i]$` 均可。

- MATH-1.11：为了统一且书写方便，复杂度分析时大 $O$ 记号请直接使用 `$O()$` 而不是 `$\mathcal O()$`。

- MATH-1.12：在表示等价关系时，请使用 `$\iff$`，效果 $\iff$，而不是 `$\Leftrightarrow$`，效果 $\Leftrightarrow$。
...
| `$R,N^*$`（集合）                | $R,N^*$           | `$\mathbf{R}$, $\mathbf{N}^*$`          | $\mathbf{R}$，$\mathbf{N}^*$         | |`$\emptyset$`               | $\emptyset$       |`$\varnothing$`                         | $\varnothing$                       | |`$size$`                    | $size$            |`$\textit{size}$`                        | $\textit{size}$                     |
...
- MATH-2.6：若要输出 LaTeX 符号 $\rm{\LaTeX}$，请用 `$\rm{\LaTeX}$`，而不是 `mathrm`；（`\LaTeX` 在 TeX 排版系统中是一个不能用于数学模式下的命令，而 `\mathrm` 又不能在普通模式下使用；另外，`\text` 命令虽然在 TeX 上正常输出，但是在 MathJax 中 `\text` 命令的参数会被原样输出，而不是按命令转义）；
...
- CODE-1.2：由于 Markdown 文档中内嵌的代码难以实现自动化测试，所以推荐使用例题代码的格式插入片段代码。有如下两种方式可供选择：
  ...
  2. 在代码文件中用 `// --8<-- [start:name]` 和 `// --8<-- [end:name]` 标记片段的起始和结束，之后在文档中用 `--8<-- "你的代码路径:name"` 即可插入该片段。参见 [Snippet Sections](https://facelessuser.github.io/pymdown-extensions/extensions/snippets/#snippet-sections)<sup>[[存档](https://web.archive.org/web/20211212193613/https://facelessuser.github.io/pymdown-extensions/extensions/snippets/#snippet-sections)]</sup>。

     示例：[前缀和](https://github.com/OI-wiki/OI-wiki/blob/c7cf6d6de13b44757f1d0528e952349beb921f8a/docs/basic/prefix-sum.md?plain=1#L37)。正文中不需要引用 [prefix-sum_1.cpp](https://github.com/OI-wiki/OI-wiki/blob/c7cf6d6de13b44757f1d0528e952349beb921f8a/docs/basic/code/prefix-sum/prefix-sum_1.cpp) 中的测试部分，所以选择插入主要的代码片段。

     **注意**：不要使用 [Snippet Lines](https://facelessuser.github.io/pymdown-extensions/extensions/snippets/#snippet-lines)<sup>[[存档](https://web.archive.org/web/20171111164017/https://facelessuser.github.io/pymdown-extensions/extensions/snippets/#snippet-lines)]</sup> 语法。
...
- CODE-3.4：不要使用 [代用记号](https://en.cppreference.com/w/cpp/language/operator_alternative#Alternative_tokens)<sup>[[存档](https://web.archive.org/web/20211210012037/https://en.cppreference.com/w/cpp/language/operator_alternative#Alternative_tokens)]</sup>。
- CODE-3.5：使用 [聚合初始化](https://en.cppreference.com/w/cpp/language/aggregate_initialization)<sup>[[存档](https://web.archive.org/web/20231225132302/https://en.cppreference.com/w/cpp/language/aggregate_initialization)]</sup> 时，`object{args}` 不可写成 `(object){args}`。
- CODE-3.6：使用 [运算符重载](https://en.cppreference.com/w/cpp/language/operators)<sup>[[存档](https://web.archive.org/web/20201112040736/https://en.cppreference.com/w/cpp/language/operators)]</sup> 时注意格式，如重载比较运算符时，若使用成员函数写法，则不可省略 `const` 限定符。
- CODE-3.7：不要使用类似 `#define int long long` 的宏定义。
- CODE-3.8：若您需要使用 C 风格的 [有格式输入/输出](https://en.cppreference.com/w/cpp/io/c#Formatted_input.2Foutput)<sup>[[存档](https://web.archive.org/web/20221130132418/https://en.cppreference.com/w/cpp/io/c#Formatted_input.2Foutput)]</sup>，请特别留意格式指示符的写法：如 `size_t` 对应 `%zu`，`ptrdiff_t` 对应 `%td`。例如输出某 STL 容器的大小时，代码应类似 `printf("%zu", container.size());`。
- CODE-3.9：由于当前测试环境 libstdc++ 的 `<chrono>` 库有 [BUG](https://github.com/actions/runner-images/issues/8659)<sup>[[存档](https://web.archive.org/web/20231220202218/https://github.com/actions/runner-images/issues/8659)]</sup>，所以请避免使用 `<chrono>` 库。
- CODE-3.10：由于 `long` 与 `unsigned long` 在某些测试环境下为 32 位，而在另一些测试环境下为 64 位，为确保各平台代码行为一致，故不推荐使用这两种类型。推荐使用 [定宽整数类型](../lang/var.md#%E5%AE%9A%E5%AE%BD%E6%95%B4%E6%95%B0%E7%B1%BB%E5%9E%8B)。
- CODE-3.11：不建议使用 `__gcd`、`__int128`、`__builtin_` 系列函数等非标准内容。如果您需要使用，则需确保您的代码能通过全平台测试，如 [此代码](https://github.com/OI-wiki/OI-wiki/blob/4af83d6db6017f4c36db6d4a7583bbc3f6257484/docs/ds/code/tree-decompose/tree-decompose_1.cpp#L24-L47) 提供了 libstdc++ 中 [std::bitset](../lang/csl/bitset.md) 特有成员函数 `_Find_first()` 的全平台实现。
...
- [标点符号用法（GB/T 15834—2011）](http://www.moe.gov.cn/jyb_sjzl/ziliao/A19/201001/W020190128580990138234.pdf)<sup>[[存档](https://web.archive.org/web/20250617134836/http://www.moe.gov.cn/jyb_sjzl/ziliao/A19/201001/W020190128580990138234.pdf)]</sup>
- [维基百科：格式手册/标点符号](https://zh.wikipedia.org/wiki/Wikipedia:%E6%A0%BC%E5%BC%8F%E6%89%8B%E5%86%8C/%E6%A0%87%E7%82%B9%E7%AC%A6%E5%8F%B7)<sup>[[存档](https://web.archive.org/web/20231204202214/https://zh.wikipedia.org/wiki/Wikipedia:%E6%A0%BC%E5%BC%8F%E6%89%8B%E5%86%8C/%E6%A0%87%E7%82%B9%E7%AC%A6%E5%8F%B7)]</sup>
- [中文文案排版指北（简体中文版）](https://mazhuang.org/wiki/chinese-copywriting-guidelines/)<sup>[[存档](https://web.archive.org/web/20160513225707/https://mazhuang.org/wiki/chinese-copywriting-guidelines/)]</sup>
- [中文文案风格指南 - PDFE GUIDELINE](https://pdfe.github.io/GUIDELINE/#/others/copywriter)<sup>[[存档](https://web.archive.org/web/20201127070738/https://pdfe.github.io/GUIDELINE/#/others/copywriter)]</sup>
- [一份（不太）简短的 LATEX2ε 介绍或 106 分钟了解 LATEX2ε](https://github.com/CTeX-org/lshort-zh-cn/releases)<sup>[[存档](https://web.archive.org/web/20241231195822/https://github.com/CTeX-org/lshort-zh-cn/releases)]</sup>
- [中文出版物夹用英文的编辑规范](https://www.nppa.gov.cn/xxgk/fdzdgknr/hybz/202210/t20221004_445147.html)<sup>[[存档](https://web.archive.org/web/20250413090857/https://www.nppa.gov.cn/xxgk/fdzdgknr/hybz/202210/t20221004_445147.html)]</sup>

## 参考资料与注释

[^note3]: 折叠框：参见 [Collapsible Blocks](https://squidfunk.github.io/mkdocs-material/reference/admonitions/#collapsible-blocks)<sup>[[存档](https://web.archive.org/web/20250907050702/https://squidfunk.github.io/mkdocs-material/reference/admonitions/#collapsible-blocks)]</sup>，有时我们也用「Details 语法」指代该语法，因其从功能上与 HTML 中的 [`<details>` 元素](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/details)<sup>[[存档](https://web.archive.org/web/20250909210049/https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/details)]</sup> 功能一致。
[^note6]: 选项卡：参见 [Content tabs](https://squidfunk.github.io/mkdocs-material/reference/content-tabs)<sup>[[存档](https://web.archive.org/web/20211128021452/https://squidfunk.github.io/mkdocs-material/reference/content-tabs)]</sup>。
...
[^webarchive]: [Save Page in Internet Archive](https://web.archive.org/save/)<sup>[[存档](https://web.archive.org/web/20191231201910/https://web.archive.org/save/)]</sup>
[^note1]: （冒号）表示总结上文。
[^note2]: 科学技术名称的英文全称与其缩略形式间，应使用英文逗号。中文句子内夹用了用以注释、补充或说明的英文句子或语段，该英文句子或语段用中文圆括号标示。
[^ref1]: [cstdio stdio.h namespace](https://stackoverflow.com/questions/10460250/cstdio-stdio-h-namespace)<sup>[[存档](https://web.archive.org/web/20220702063456/https://stackoverflow.com/questions/10460250/cstdio-stdio-h-namespace)]</sup>
