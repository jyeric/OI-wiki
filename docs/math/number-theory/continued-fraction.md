...
此时的计算过程实际上是对 $p$ 和 $q$ 做 [辗转相除法](./gcd.md#%E6%AC%A7%E5%87%A0%E9%87%8C%E5%BE%97%E7%AE%97%E6%B3%95)。这也说明，对于有理数 $r=\dfrac{p}{q}$，连分数表示的长度是 $O(\log\min{p, q})$ 的。计算有理数 $\dfrac{p}{q}$ 的复杂度也是 $O(\log\min{p, q})$ 的。
...
??? note "解答"
    ...
    虽然这个问题通常是用 [扩展欧几里得算法](./bezouts.md#%E4%B8%A4%E4%B8%AA%E5%8F%98%E9%87%8F%E7%9A%84%E6%83%85%E5%BD%A2) 解决的，但是同样可以通过连分数求解。
...
这个定理也可以看作是 [Dirichlet 逼近定理](https://en.wikipedia.org/wiki/Dirichlet%27s_approximation_theorem)<sup>[[存档](https://web.archive.org/web/20250822182743/https://en.wikipedia.org/wiki/Dirichlet%27s_approximation_theorem)]</sup> 的推论。这几乎已经是最好的结果了。不等式右侧分母中的指数 $2$ 已经不能再改进，但是常数上可以做得更好。Hurwitz 定理说明，不等式右侧可以缩小到 $\dfrac{1}{\sqrt{5}q^2}$，且这是最好的界。
...
??? note "几何解释"
    ...
    - 直线 $y=\xi x$ 的方向向量是 $\vec\xi=(1,\xi)$。利用 [叉积](../linear-algebra/product.md#%E4%BA%8C%E7%BB%B4%E5%90%91%E9%87%8F%E7%9A%84%E6%83%85%E5%BD%A2) $(x_1,y_1)\times(x_2,y_2)=x_1y_2-x_2y_1$ 的概念，可以通过 $\vec\xi\times\vec\nu=p-q\xi$ 的正负判断点在直线上方还是下方。因而，在直线上方的点就对应着大于等于 $\xi$ 的分数，在直线下方的点就对应着小于等于 $\xi$ 的分数。叉积的绝对值 $|\vec\xi\times\vec\nu|$ 正比于点 $\vec\nu$ 与直线 $y=\xi x$ 的距离
...
这样得到的上下两个凸包称为 Klein 多边形。在高维空间内也可以做类似定义，得到 [Klein 多面体](https://en.wikipedia.org/wiki/Klein_polyhedron)<sup>[[存档](https://web.archive.org/web/20240927164800/https://en.wikipedia.org/wiki/Klein_polyhedron)]</sup>（Klein polyhedron），它可以将连分数的概念推广到高维空间内。
...
??? note "解答"
    ...
    首先将两个连分数表示都转化成末尾是 $1$ 的形式。不妨设题目所给的已经是这样形式的连分数，即 $\alpha_n=\beta_m=1$。因为偶数位置（下标从 $0$ 开始）是向右移动的步数，奇数位置是向左移动的步数，所以，$\alpha<\beta$ 当且仅当按照 [字典序](../../string/basic.md#%E5%AD%97%E5%85%B8%E5%BA%8F) 比较时，有
...
???+ note "分式线性变换的性质"
    ...
    1. 分式线性变换的复合 $L_1\circ L_2$ 和逆变换 $L_1^{-1}$ 仍然是分式线性变换，即全体分式线性变换构成 [群](../algebra/basic.md#%E7%BE%A4)；
...
???+ example "[DMOPC '19 Contest 7 P4 - Bob and Continued Fractions](https://dmoj.ca/problem/dmopc19c7p4)<sup>[[存档](https://web.archive.org/web/20201204202902/https://dmoj.ca/problem/dmopc19c7p4)]</sup>"
...
??? note "证明"
    ...
    $$
    \begin{aligned}
    \dfrac{p_\ell}{p_{\ell-1}} &= [a_\ell,\cdots,a_1,a_0] = \dfrac{p'*\ell}{q'*\ell},\
    \dfrac{q_\ell}{q_{\ell-1}} &= [a_\ell,\cdots,a_1] = \dfrac{p'*{\ell-1}}{q'*{\ell-1}}.
    \end{aligned}
    $$
    ...
    $$
    p'*\ell = p*\ell,\ q'*\ell = p*{\ell-1},\ p'*{\ell-1}=q*\ell,\ q'*{\ell-1} = q*{\ell-1}.
    $$
    ...
    $$
    q'*\ell(x')^2+(q'*{\ell-1}-p'*\ell)x'-p'*{\ell-1} = 0.
    $$
...
??? note "解答"
    ...
    进而，考虑子问题的解法。对比于原问题，子问题相当于将 $x$ 的上界修改为 $N'$，并求出上凸壳中与原点相邻的第一个整点。记子问题的解为 $(q,p)$。那么，$p$ 与 $q$ 必然是互素的（否则不是第一个整点），且与原点连线的斜率 $\dfrac{p}{q}$ 是所有位于直线 $y=rx$ 下方且横坐标不超过 $N'$ 的整点中最大的（否则不在凸包上）。结合前文的 [几何解释](#%E5%87%A0%E4%BD%95%E8%A7%A3%E9%87%8A) 可知，这样的点 $(x,y)$ 必然对应于 $r$ 的一个下中间分数。因为分母越大的下中间分数离 $r$ 越近，所以子问题的解 $(q,p)$ 对应着所有分母不超过 $N'$ 的下中间分数中分母最大的那个。
...
??? note "解答"
    ...
    结合前文的 [几何解释](#%E5%87%A0%E4%BD%95%E8%A7%A3%E9%87%8A) 可知，只要后面的常数项小于 $1$，那么满足这个不等式的整点 $(\Delta x,\Delta y)$ 中横坐标最小的，一定对应着某个上中间分数。这是因为它是所有分母不超过它的分母的分数中，从上方逼近某个实数效果最好的，这只能是上中间分数。而每次添加增量后，都会导致 $\Delta y$ 的上界变得更紧，这意味着必须考察分母更大的上中间分数。
...
???+ example "[NAIPC 2019 - It's a Mod, Mod, Mod, Mod World](https://open.kattis.com/problems/itsamodmodmodmodworld)<sup>[[存档](https://web.archive.org/web/20210731212020/https://open.kattis.com/problems/itsamodmodmodmodworld)]</sup>"
...
??? note "解答"
    ...
    在 $(q,k)$ 所在的平面坐标系内，这相当于要找到 $q\in[1,n]$ 时在直线 $qr-km=0$ 下方最接近它的整点，因为余数 $qr\bmod m$ 就正比于整点与直线的距离。结合前文的 [几何解释](#%E5%87%A0%E4%BD%95%E8%A7%A3%E9%87%8A) 可知，这样的整点必然对应着有理分数 $\dfrac{r}{m}$ 的某个下中间分数。算法复杂度是 $O(\log\min{r,m})$。
...
- [UVa OJ - Continued Fractions](https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=775)<sup>[[存档](https://web.archive.org/web/20250903030700/https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=775)]</sup>
- [ProjectEuler+ #64: Odd period square roots](https://www.hackerrank.com/contests/projecteuler/challenges/euler064/problem)<sup>[[存档](https://web.archive.org/web/20231130114717/https://www.hackerrank.com/contests/projecteuler/challenges/euler064/problem)]</sup>
- [「LibreOJ NOI Round #2」单枪匹马](https://loj.ac/p/573)<sup>[[存档](https://web.archive.org/web/20230607225825/https://loj.ac/p/573)]</sup>
- [Codeforces Round #184 (Div. 2) - Continued Fractions](https://codeforces.com/contest/305/problem/B)<sup>[[存档](https://web.archive.org/web/20221217112845/https://codeforces.com/contest/305/problem/B)]</sup>
- [Codeforces Round #201 (Div. 1) - Doodle Jump](https://codeforces.com/contest/346/problem/E)<sup>[[存档](https://web.archive.org/web/20221217144309/https://codeforces.com/contest/346/problem/E)]</sup>
- [Codeforces Round #325 (Div. 1) - Alice, Bob, Oranges and Apples](https://codeforces.com/contest/585/problem/C)<sup>[[存档](https://web.archive.org/web/20240804173154/https://codeforces.com/contest/585/problem/C)]</sup>
- [POJ Founder Monthly Contest 2008.03.16 - A Modular Arithmetic Challenge](http://poj.org/problem?id=3530)<sup>[[存档](https://web.archive.org/web/20250811134043/http://poj.org/problem?id=3530)]</sup>
- [2019 Multi-University Training Contest 5 - fraction](http://acm.hdu.edu.cn/showproblem.php?pid=6624)<sup>[[存档](https://web.archive.org/web/20241127022559/http://acm.hdu.edu.cn/showproblem.php?pid=6624)]</sup>
- [SnackDown 2019 Elimination Round - Election Bait](https://www.codechef.com/SNCKEL19/problems/EBAIT)<sup>[[存档](https://web.archive.org/web/20211018141904/https://www.codechef.com/SNCKEL19/problems/EBAIT)]</sup>
- [Luogu P5179. Fraction](https://www.luogu.com.cn/problem/P5179)<sup>[[存档](https://web.archive.org/web/20231013090541/https://www.luogu.com.cn/problem/P5179)]</sup>
- [Luogu P7739. [NOI2021] 密码箱](https://www.luogu.com.cn/problem/P7739)<sup>[[存档](https://web.archive.org/web/20250815173558/https://www.luogu.com.cn/problem/P7739)]</sup>
...
- 朱尧辰，王连祥《丢番图逼近引论》
- [FatFish 的博客 - 连分数入门](https://chaoli.club/index.php/2756)<sup>[[存档](https://web.archive.org/web/20170707150612/https://chaoli.club/index.php/2756)]</sup>
- [Simple continued fraction - Wikipedia](https://en.wikipedia.org/wiki/Simple_continued_fraction)<sup>[[存档](https://web.archive.org/web/20231223234945/https://en.wikipedia.org/wiki/Simple_continued_fraction)]</sup>
- [Periodic continued fraction - Wikipedia](https://en.wikipedia.org/wiki/Periodic_continued_fraction)<sup>[[存档](https://web.archive.org/web/20250829210421/https://en.wikipedia.org/wiki/Periodic_continued_fraction)]</sup>
- [Gosper's original notes on continued fraction arithmetic algorithms](https://perl.plover.com/yak/cftalk/INFO/gosper.txt)<sup>[[存档](https://web.archive.org/web/20231129050403/https://perl.plover.com/yak/cftalk/INFO/gosper.txt)]</sup>
- [Understanding Bill Gosper's continued fraction arithmetic (implemented in Python)](https://hsinhaoyu.github.io/cont_frac/)<sup>[[存档](https://web.archive.org/web/20221117075204/https://hsinhaoyu.github.io/cont_frac/)]</sup>

**本页面主要内容译自博文 [Continued fractions](https://cp-algorithms.com/algebra/continued-fractions.html)<sup>[[存档](https://web.archive.org/web/20230929180416/https://cp-algorithms.com/algebra/continued-fractions.html)]</sup>，版权协议为 CC-BY-SA 4.0，内容有改动。**
...
[^period-surd]: 证明见 [维基百科页面](https://en.wikipedia.org/wiki/Periodic_continued_fraction#Length_of_the_repeating_block)<sup>[[存档](https://web.archive.org/web/20181130122712/https://en.wikipedia.org/wiki/Periodic_continued_fraction#Length_of_the_repeating_block)]</sup> 的参考文献。
