...
??? note "证明"
    ...
    首先由 [Fermat 小定理](./fermat.md#%E8%B4%B9%E9%A9%AC%E5%B0%8F%E5%AE%9A%E7%90%86) 有 $a^{p-1}\equiv 1\pmod p$，故
    ...
    由 [同余方程的定理 5](./congruence-equation.md#%E5%AE%9A%E7%90%86-5) 可知，$a$ 是模 $p$ 的二次剩余当且仅当 $a^{(p-1)/2}\equiv 1\pmod p$. 进而 $a$ 是模 $p$ 的非二次剩余当且仅当 $a^{(p-1)/2}\equiv -1\pmod p$.
...
??? note "证明"
    ...
    注意到 $\dfrac{p-1}{2}\mid (p-1)$，由 [同余方程的定理 6](./congruence-equation.md#%E5%AE%9A%E7%90%86-6) 可知 $a^{\frac{p-1}{2}}\equiv 1\pmod p$ 有 $\dfrac{p-1}{2}$ 个解。所以模 $p$ 意义下二次剩余和二次非剩余均有 $\dfrac{p-1}{2}$ 个。
...
下表为部分 Legendre 符号的值（From [Wikipedia](https://en.wikipedia.org/wiki/Legendre_symbol#Table_of_values)<sup>[[存档](https://web.archive.org/web/20241230204426/https://en.wikipedia.org/wiki/Legendre_symbol#Table_of_values)]</sup>）
...
3. （[完全积性](./basic.md#%E7%A7%AF%E6%80%A7%E5%87%BD%E6%95%B0)）对任意整数 $a_1,a_2$，
...
??? note "证明"
    ...
    1. 由 [Legendre 符号的定义](#legendre-%E7%AC%A6%E5%8F%B7) 和 [Euler 判别法](#euler-%E5%88%A4%E5%88%AB%E6%B3%95) 易得。
    ...
    4. 参见 [二次互反律](#%E4%BA%8C%E6%AC%A1%E4%BA%92%E5%8F%8D%E5%BE%8B)

基于如上性质，若对任意奇素数 $p$ 和 $q$，$\left(\dfrac{p}{q}\right)$ 的值均可计算，则我们就可以对任意合法情况计算 Legendre 符号的值。接下来我们有一个优美的定理，这个定理巧妙地在 $\left(\dfrac{p}{q}\right)$ 和 $\left(\dfrac{q}{p}\right)$ 之间建立起了联系，使得我们能用类似 [辗转相除法](./gcd.md#%E6%AC%A7%E5%87%A0%E9%87%8C%E5%BE%97%E7%AE%97%E6%B3%95) 的思路完成计算。
...
??? note "证明"
    ...
    从而由 Legendre 符号的 [性质 1](#%E6%80%A7%E8%B4%A8) 即得证。
...
另外，我们还可以证明诸如「形如 $4k+1$ 的素数有无限多个」之类的结论，这一类结论实际上是 [Dirichlet 定理](https://en.wikipedia.org/wiki/Dirichlet%27s_theorem_on_arithmetic_progressions)<sup>[[存档](https://web.archive.org/web/20250906000456/https://en.wikipedia.org/wiki/Dirichlet%27s_theorem_on_arithmetic_progressions)]</sup> 的简单推论。
...
???+ abstract "Jacobi 符号"
    ...
    其中等式右端的 $\left(\frac{a}{p_i}\right)$ 为 [Legendre 符号](#legendre-%E7%AC%A6%E5%8F%B7)。另外对整数 $a$ 有 $\left(\dfrac{a}{1}\right)=1.$
...
我们还可以把模数进一步推广为 **整数**（只需补充 $\left(\dfrac{a}{-1}\right)$、$\left(\dfrac{a}{0}\right)$ 和 $\left(\dfrac{a}{2}\right)$ 的定义），这样就得到了 [Kronecker 符号](https://en.wikipedia.org/wiki/Kronecker_symbol)<sup>[[存档](https://web.archive.org/web/20141026045510/https://en.wikipedia.org/wiki/Kronecker_symbol)]</sup>。
...
??? note "计算方法"
    ...
    不熟悉 [多项式环](../algebra/ring-theory.md#%E5%A4%9A%E9%A1%B9%E5%BC%8F%E7%8E%AF) 的读者，可以简单理解为该集合的元素都具有形式 $a_0+a_1x$ 且 $a_0,a_1\in\mathbf F_p$，且遵循如下运算法则：
...
- [洛谷 P5491【模板】二次剩余](https://www.luogu.com.cn/problem/P5491)<sup>[[存档](https://web.archive.org/web/20241204005931/https://www.luogu.com.cn/problem/P5491)]</sup>
- [「Timus 1132」Square Root](https://acm.timus.ru/problem.aspx?space=1&num=1132)<sup>[[存档](https://web.archive.org/web/20250906014649/https://acm.timus.ru/problem.aspx?space=1&num=1132)]</sup>

## 参考资料与注释


1. [Quadratic residue - Wikipedia](https://en.wikipedia.org/wiki/Quadratic_residue)<sup>[[存档](https://web.archive.org/web/20201112032212/https://en.wikipedia.org/wiki/Quadratic_residue)]</sup>
2. [Euler's criterion - Wikipedia](https://en.wikipedia.org/wiki/Euler%27s_criterion)<sup>[[存档](https://web.archive.org/web/20221114170825/https://en.wikipedia.org/wiki/Euler%27s_criterion)]</sup>

[^ref5]: [Proofs of quadratic reciprocity - Wikipedia](https://en.wikipedia.org/wiki/Proofs_of_quadratic_reciprocity)<sup>[[存档](https://web.archive.org/web/20170109215114/https://en.wikipedia.org/wiki/Proofs_of_quadratic_reciprocity)]</sup>
[^ref3]: A. Menezes, P. van Oorschot and S. Vanstone. Handbook of Applied Cryptography, 1996.
[^ref4]: Alin Bostan, Ryuhei Mori.[A Simple and Fast Algorithm for Computing the N-th Term of a Linearly Recurrent Sequence](https://arxiv.org/abs/2008.08822)<sup>[[存档](https://web.archive.org/web/20240829093607/https://arxiv.org/abs/2008.08822)]</sup>.
