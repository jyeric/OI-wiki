...
OI 中有关线性基的应用一般只涉及两类线性空间：$n$ 维实线性空间 $\mathbf{R}^n$ 和 $n$ 维 [布尔域](https://en.wikipedia.org/wiki/Boolean_domain)<sup>[[存档](https://web.archive.org/web/20201016173541/https://en.wikipedia.org/wiki/Boolean_domain)]</sup> 线性空间 $\mathbf{Z}_2^n$，我们会在 [应用](#%E5%BA%94%E7%94%A8) 一节中详细介绍。若您不熟悉线性代数，则推荐从应用部分开始阅读。
...
任意有限维线性空间 $V$ 的基都可以通过 [Schmidt 正交化](https://en.wikipedia.org/wiki/Gram%E2%80%93Schmidt_process)<sup>[[存档](https://web.archive.org/web/20250903195454/https://en.wikipedia.org/wiki/Gram%E2%80%93Schmidt_process)]</sup> 变换为正交基。
...
- 将线性基 $\beta$ 中的向量 $\beta_j$ 利用 [贪心法](#%E8%B4%AA%E5%BF%83%E6%B3%95) 尝试插入到 $\alpha$ 中，并初始化线性基的交 $\gamma$ 为空集；
...
??? note "对算法的解释"
    ...
    设合并后的线性基为 ${\alpha_1,\cdots,\alpha_m,\beta'*{j_1},\cdots,\beta'*{j_\ell}}$，其中，$\beta'*{j_k}$ 是插入 $\beta*{j_k}$ 时最后得到的向量。那么，${\alpha_1,\cdots,\alpha_m,\beta_{j_1},\cdots,\beta_{j_\ell}}$ 同样是一组合并后的线性基。记 $\beta^+$ 为集合 ${\beta_{j_1},\cdots,\beta_{j_\ell}}$，则合并后的基可以写作 $\alpha\cup\beta^+$。而且，和空间中的每个向量 $c$ 都可以唯一地表示成
...
算法中的构造线性基的方法可以是 [贪心法](#%E8%B4%AA%E5%BF%83%E6%B3%95) 或 [高斯消元法](#%E9%AB%98%E6%96%AF%E6%B6%88%E5%85%83%E6%B3%95)，只要保证 $\gamma$ 中的线性基组成行阶梯型矩阵即可。
...
??? note "正确性证明"
    ...
    根据 [线性映射的相关定理](./linear-mapping.md#%E7%BA%BF%E6%80%A7%E6%98%A0%E5%B0%84%E7%9A%84%E6%A0%B8%E7%A9%BA%E9%97%B4%E4%B8%8E%E5%83%8F%E7%A9%BA%E9%97%B4)，有 $\dim H = \dim\pi(H)+\dim\ker\pi = \dim(U+W)+\dim(U\cap W)$。
...
基于上文提到的 [贪心法](#%E8%B4%AA%E5%BF%83%E6%B3%95) 构造线性基，前缀线性基在构造过程中做了如下调整：
...
也就是说，如果当前位可以通过较新的向量表示，就直接用较新的向量；否则，保留原来的向量。在更新位置 $x$ 的向量时，不能将异或的结果 $a_x\oplus v$ 存入位置 $x$，因为异或的结果 $a_x\oplus v$ 的时间戳为 $\min{t(a_x)=t(v)}=t(a_x)$，小于要添加的变量 $v$ 的时间戳 $t(v)$。同样的原因，[高斯消元法](#%E9%AB%98%E6%96%AF%E6%B6%88%E5%85%83%E6%B3%95) 构造线性基的过程中向上更新时可能会破坏时间戳的性质，所以不再适用于构造前缀线性基。
...
- [Luogu P3812【模板】线性基](https://www.luogu.com.cn/problem/P3812)<sup>[[存档](https://web.archive.org/web/20231203012416/https://www.luogu.com.cn/problem/P3812)]</sup>
- [Acwing 3164. 线性基](https://www.acwing.com/problem/content/description/3167)
- [SGU 275 to xor or not xor](https://codeforces.com/problemsets/acmsguru/problem/99999/275)<sup>[[存档](https://web.archive.org/web/20191227084649/https://codeforces.com/problemsets/acmsguru/problem/99999/275)]</sup>
- [HDU 3949 XOR](https://acm.hdu.edu.cn/showproblem.php?pid=3949)<sup>[[存档](https://web.archive.org/web/20220627083534/https://acm.hdu.edu.cn/showproblem.php?pid=3949)]</sup>
- [HDU 6579 Operation](https://acm.hdu.edu.cn/showproblem.php?pid=6579)<sup>[[存档](https://web.archive.org/web/20220123141209/https://acm.hdu.edu.cn/showproblem.php?pid=6579)]</sup>
- [Luogu P4151 [WC2011] 最大 XOR 和路径](https://www.luogu.com.cn/problem/P4151)<sup>[[存档](https://web.archive.org/web/20221006195124/https://www.luogu.com.cn/problem/P4151)]</sup>
- [Library Checker - Intersection of $\mathbf F_2$ vector spaces](https://judge.yosupo.jp/problem/intersection_of_f2_vector_spaces)<sup>[[存档](https://web.archive.org/web/20250621051751/https://judge.yosupo.jp/problem/intersection_of_f2_vector_spaces)]</sup>
- [AtCoder Grand Contest 045 A - Xor Battle](https://atcoder.jp/contests/agc045/tasks/agc045_a)<sup>[[存档](https://web.archive.org/web/20211207033713/https://atcoder.jp/contests/agc045/tasks/agc045_a)]</sup>
- [Codeforces 1100F Ivan and Burgers](https://codeforces.com/problemset/problem/1100/F)<sup>[[存档](https://web.archive.org/web/20200809080120/https://codeforces.com/problemset/problem/1100/F)]</sup>
- [Luogu P3292 [SCOI2016] 幸运数字](https://www.luogu.com.cn/problem/P3292)<sup>[[存档](https://web.archive.org/web/20200929075324/https://www.luogu.com.cn/problem/P3292)]</sup>

## 参考资料与注释


1. 丘维声，高等代数（下）。清华大学出版社。
2. [Basis (linear algebra) - Wikipedia](https://en.wikipedia.org/wiki/Basis_%28linear_algebra%29)<sup>[[存档](https://web.archive.org/web/20221224222400/https://en.wikipedia.org/wiki/Basis_%28linear_algebra%29)]</sup>
3. [Vector Basis -- from Wolfram MathWorld](https://mathworld.wolfram.com/VectorBasis.html)<sup>[[存档](https://web.archive.org/web/20221129225424/https://mathworld.wolfram.com/VectorBasis.html)]</sup>
4. [Zassenhaus algorithm - Wikipedia](https://en.wikipedia.org/wiki/Zassenhaus_algorithm)<sup>[[存档](https://web.archive.org/web/20210506202137/https://en.wikipedia.org/wiki/Zassenhaus_algorithm)]</sup>

[^existence_basis]: [Proof that every vector space has a basis](https://en.wikipedia.org/wiki/Basis_%28linear_algebra%29#Proof_that_every_vector_space_has_a_basis)<sup>[[存档](https://web.archive.org/web/20241228083150/https://en.wikipedia.org/wiki/Basis_%28linear_algebra%29#Proof_that_every_vector_space_has_a_basis)]</sup>
