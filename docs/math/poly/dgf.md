...
如果序列 $f$ 满足积性（是 [积性函数](../number-theory/basic.md#%E7%A7%AF%E6%80%A7%E5%87%BD%E6%95%B0)）：$\forall i\perp j, ; f_{ij} = f_i f_j$，那么其 DGF 可以由质数幂处的取值表示：
...
对于两个序列 $f, g$，其 DGF 之积对应的是两者的 [狄利克雷卷积](#dirichlet-%E5%8D%B7%E7%A7%AF) 序列的 DGF：
...
以 [洛谷 P3768 简单的数学题](../number-theory/du.md#%E9%97%AE%E9%A2%98%E4%BA%8C) 为例，我们要对 $f_i = i^2\varphi(i)$ 构造一个满足上述条件的积性序列 $g$。由于 $f$ 是积性的，考虑其 DGF
...
[^1]: <https://en.wikipedia.org/wiki/Generating_function#Dirichlet_series_generating_functions_(DGFs)><sup>[[存档](<https://web.archive.org/web/20191028162617/https://en.wikipedia.org/wiki/Generating_function#Dirichlet_series_generating_functions_(DGFs)>)]</sup>
