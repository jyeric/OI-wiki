...
$$
\begin{aligned}
f\left(x\right)f^{-1}*{0}\left(x\right)&\equiv 1 &\pmod{x^{\left\lceil\frac{n}{2}\right\rceil}}\
f\left(x\right)f^{-1}\left(x\right)&\equiv 1 &\pmod{x^{\left\lceil\frac{n}{2}\right\rceil}}\
f^{-1}\left(x\right)-f^{-1}*{0}\left(x\right)&\equiv 0 &\pmod{x^{\left\lceil\frac{n}{2}\right\rceil}}
\end{aligned}
$$
...
1. 有标号简单无向连通图计数：[「POJ 1737」Connected Graph](http://poj.org/problem?id=1737)<sup>[[存档](https://web.archive.org/web/20240921090622/http://poj.org/problem?id=1737)]</sup>
...
??? info " 洛谷模板题 [P5205【模板】多项式开根](https://www.luogu.com.cn/problem/P5205) 参考代码 "
...
1. [「Codeforces Round #250」E. The Child and Binary Tree](https://codeforces.com/contest/438/problem/E)<sup>[[存档](https://web.archive.org/web/20230720003400/https://codeforces.com/contest/438/problem/E)]</sup>
...
发现若能消除 $R\left(x\right)$ 的影响则可直接 [多项式求逆](#%E5%A4%9A%E9%A1%B9%E5%BC%8F%E6%B1%82%E9%80%86) 解决。
...
=== "多项式对数函数"
    ...
    首先，对于多项式 $f(x)$，若 $\ln{f(x)}$ 存在，则由其 [定义](./intro.md#%E5%A4%8D%E5%90%88)，其必须满足：
...
首先由 [Euler's formula](../complex.md#%E6%AC%A7%E6%8B%89%E5%85%AC%E5%BC%8F) $\left(\mathrm{e}^{\mathrm{i}x} = \cos{x} + \mathrm{i}\sin{x}\right)$ 可以得到 [三角函数的另一个表达式](https://en.wikipedia.org/wiki/Trigonometric_functions#Relationship_to_exponential_function_and_complex_numbers)<sup>[[存档](https://web.archive.org/web/20241225133134/https://en.wikipedia.org/wiki/Trigonometric_functions#Relationship_to_exponential_function_and_complex_numbers)]</sup>：
...
[^ref1]: [Elementary function——Wikipedia](https://en.wikipedia.org/wiki/Elementary_function)<sup>[[存档](https://web.archive.org/web/20250830201105/https://en.wikipedia.org/wiki/Elementary_function)]</sup>
