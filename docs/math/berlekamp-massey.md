...
设 $\boldsymbol{f}*i$ 是一个 $n$ 维列向量，并且转移满足 $\boldsymbol{f}_i = A \boldsymbol{f}*{i - 1}$，则可以发现 ${\boldsymbol{f}_i}$ 是一个不超过 $n$ 阶的线性递推向量列。（证明略）


我们可以直接暴力求出 $\boldsymbol{f}*0 \dots \boldsymbol{f}*{2n - 1}$，然后用前面提到的做法求出 ${\boldsymbol{f}_i}$ 的最短递推式，再调用 [常系数齐次线性递推](./poly/linear-recurrence.md) 即可。
...
1. [LibreOJ #163. 高斯消元 2](https://loj.ac/p/163)<sup>[[存档](https://web.archive.org/web/20220626133144/https://loj.ac/p/163)]</sup>
