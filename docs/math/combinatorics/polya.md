...
相关阅读：[Burnside 引理](../algebra/group-theory.md#burnside-%E5%BC%95%E7%90%86)
...
??? example "解答和分析"
    ...
    这里，$[\cdot]$ 称为 [Iverson 括号](https://mathworld.wolfram.com/IversonBracket.html)<sup>[[存档](https://web.archive.org/web/20241112161846/https://mathworld.wolfram.com/IversonBracket.html)]</sup>，只有当命题 $P$ 为真时 $[P]$ 取值为 $1$，否则取 $0$。交换求和记号的结果中，$X^g={x\in X:gx=x}$ 是指在操作 $g$ 下保持不变的染色方案 $x$ 的集合。简言之，它是操作 $g$ 的不动点。
...
它的证明几乎就是照搬上面例子中的分析。但是，例子中用到了观察，即群 $G$ 在单个元素 $x$ 上的作用结果具有某种「周期性」，所以，这种周期重复的数目就等于能将 $x$ 变换到它自身的操作的数目。这个观察在一般的情形是正确的，但是因为群 $G$ 的结构可能很复杂，它的「周期性」未必是例子中呈现的那么直接。严格地表述这个观察，需要用到群论中的 [轨道稳定子定理（orbit-stabilizer theorem）](../algebra/group-theory.md#%E7%A8%B3%E5%AE%9A%E5%8C%96%E5%AD%90)。
...
通过生成函数可以给出这类计数问题的答案。给定置换 $g$，如果它的 [型](../permutation.md#%E7%BD%AE%E6%8D%A2%E7%9A%84%E5%9E%8B) 是 $1^{\alpha_1}2^{\alpha_2}\cdots n^{\alpha_n}$，即它有 $\alpha_k$ 个长度为 $k$ 的轮换，且对于每个轮换可以染成 $m$ 种颜色中的一种，那么生成函数
...
带权重版本的 Pólya 计数原理在组合计数问题中起到重要的作用。这里简单讨论它的应用，而更一般的讨论可以参考 [组合问题的形式化方法](../poly/symbolic-method.md#%E6%9C%89%E9%99%90%E5%88%B6%E7%9A%84%E6%9E%84%E9%80%A0)。
...
Pólya 计数相关问题的难点之一在于分析置换群的结构。这里，简单讨论常见的空间对称群的结构，并用它们的轮换指标加以描述。应当注意，对于同一个结构的空间对称群，如果考虑的作用对象的集合不同，相应的 [群作用](../algebra/group-theory.md#%E7%BE%A4%E4%BD%9C%E7%94%A8) 也就不同，因而它们的置换表示也就不同。比如说，正方体的空间对称群对于它的顶点、棱、面的作用就分别对应着正方体的顶点置换群、棱置换群和面置换群，顶点、棱、面的个数互不相同，故而这些置换群以及对应的轮换指标当然也各不相同。所以，在具体问题的求解中，不能忽视群作用的对象的指定。
...
[群](../algebra/basic.md#%E7%BE%A4) 是对所有满足这些条件的概念的抽象。对于群的结构的讨论，就是 [群论](../algebra/group-theory.md) 的主要研究内容。这里的分析主要集中在空间对称群，对它的结构的讨论也主要应用几何观点。这里给出了常见的例子，读者应当从中获得分析这类问题的常见思路。
...
根据 [置换与排列](../permutation.md#%E7%BD%AE%E6%8D%A2%E7%9A%84%E5%9E%8B) 一文的分析，它的轮换指标是
...
- [Luogu P4980【模板】Polya 定理](https://www.luogu.com.cn/problem/P4980)<sup>[[存档](https://web.archive.org/web/20241213062518/https://www.luogu.com.cn/problem/P4980)]</sup>
- [Luogu P2561 [AHOI2002] 黑白瓷砖](https://www.luogu.com.cn/problem/P2561)<sup>[[存档](https://web.archive.org/web/20231006031223/https://www.luogu.com.cn/problem/P2561)]</sup>
- [TRANSP - Transposing is Fun](https://www.spoj.com/problems/TRANSP/)<sup>[[存档](https://web.archive.org/web/20241203060459/https://www.spoj.com/problems/TRANSP/)]</sup>
- [TRANSP2 - Transposing is Even More Fun](https://www.spoj.com/problems/TRANSP2/)<sup>[[存档](https://web.archive.org/web/20191117205359/https://www.spoj.com/problems/TRANSP2/)]</sup>
- [Luogu P3307 [SDOI2013] 项链](https://www.luogu.com.cn/problem/P3307)<sup>[[存档](https://web.archive.org/web/20250809184359/https://www.luogu.com.cn/problem/P3307)]</sup>

当可以使用的颜色组合受到限制时，需要通过背包 DP 或者组合方法求解对轮换染色的方法数目。


- [Luogu P1446 [HNOI2008] Cards](https://www.luogu.com.cn/problem/P1446)<sup>[[存档](https://web.archive.org/web/20220715052946/https://www.luogu.com.cn/problem/P1446)]</sup>
- [UVA10601 Cubes](https://vjudge.net/problem/UVA-10601)
- [Luogu P4916 [MtOI2018] 魔力环](https://www.luogu.com.cn/problem/P4916)<sup>[[存档](https://web.archive.org/web/20250719052559/https://www.luogu.com.cn/problem/P4916)]</sup>
...
- [SGU 282. Isomorphism](https://codeforces.com/problemsets/acmsguru/problem/99999/282)<sup>[[存档](https://web.archive.org/web/20210617085722/https://codeforces.com/problemsets/acmsguru/problem/99999/282)]</sup>
...
- [LOJ 6538 烷基计数 加强版 加强版](https://loj.ac/p/6538)<sup>[[存档](https://web.archive.org/web/20220730122323/https://loj.ac/p/6538)]</sup>
- [LOJ 6512「雅礼集训 2018」烷烃计数](https://loj.ac/p/6512)<sup>[[存档](https://web.archive.org/web/20220526042803/https://loj.ac/p/6512)]</sup>
...
- [Pólya enumeration theorem - Wikipedia](https://en.wikipedia.org/wiki/P%C3%B3lya_enumeration_theorem)<sup>[[存档](https://web.archive.org/web/20231223205845/https://en.wikipedia.org/wiki/P%C3%B3lya_enumeration_theorem)]</sup>
- [Notes on Pólya's Enumeration Theorem](https://www.diva-portal.org/smash/get/diva2:324594/FULLTEXT01.pdf)<sup>[[存档](https://web.archive.org/web/20240413074026/https://www.diva-portal.org/smash/get/diva2:324594/FULLTEXT01.pdf)]</sup>
- [Cycle index - Wikipedia](https://en.wikipedia.org/wiki/Cycle_index)<sup>[[存档](https://web.archive.org/web/20231213100435/https://en.wikipedia.org/wiki/Cycle_index)]</sup>
