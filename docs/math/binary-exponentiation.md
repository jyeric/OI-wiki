...
给定形式 $n_{t\dots i} = (n_tn_{t-1}\cdots n_i)*2$，即 $n*{t\dots i}$ 表示将 $n$ 的前 $t - i + 1$ 位二进制位当作一个二进制数，则有如下变换：
...
???+ warning "注意"
    ...
    - 当指数很大时，需利用 [扩展欧拉定理](./number-theory/fermat.md#%E6%89%A9%E5%B1%95%E6%AC%A7%E6%8B%89%E5%AE%9A%E7%90%86) 降幂后计算。

### 计算斐波那契数


根据斐波那契数列的递推式 $F_n = F_{n-1} + F_{n-2}$，我们可以构建一个 $2\times 2$ 的矩阵来表示从 $F_i,F_{i+1}$ 到 $F_{i+1},F_{i+2}$ 的变换。于是在计算这个矩阵的 $n$ 次幂的时侯，我们使用快速幂的思想，可以在 $\Theta(\log n)$ 的时间内计算出结果。对于更多的细节参见 [斐波那契数列](./combinatorics/fibonacci.md)，矩阵快速幂的实现参见 [矩阵加速递推](../math/linear-algebra/matrix.md#%E7%9F%A9%E9%98%B5%E5%8A%A0%E9%80%9F%E9%80%92%E6%8E%A8) 中的实现。
...
简单地把这个置换取 $k$ 次幂，然后把它应用到序列上即可。时间复杂度为 $O(n \log k)$。对于更多的细节参见 [置换的复合](./permutation.md#%E5%A4%8D%E5%90%88)。
...
参考 [向量与矩阵](./linear-algebra/vector.md#%E5%90%91%E9%87%8F%E4%B8%8E%E7%9F%A9%E9%98%B5) 中的内容，每一种操作都可以用一个变换矩阵表示，一系列连续的变换可以用矩阵的乘积来表示。一个 Repeat 操作就相当于取一个矩阵的 $k$ 次幂。这样可以用 $O(m \log k)$ 的时间计算出整个变换序列最终形成的矩阵。最后将它应用到 $n$ 个点上，总复杂度 $O(n + m \log k)$。
...
我们把该图的邻接矩阵 $M$ 取 $k$ 次幂，那么 $M_{i,j}$ 就表示从 $i$ 到 $j$ 长度为 $k$ 的路径的数目。该算法的复杂度是 $O(n^3 \log k)$。有关该算法的细节请参见 [矩阵](./linear-algebra/matrix.md#%E5%AE%9A%E9%95%BF%E8%B7%AF%E5%BE%84%E7%BB%9F%E8%AE%A1) 页面。
...
但在实际使用中，此方法由于引入了更大的计算复杂度导致时间效率不优。实际编程中通常利用 [快速乘](./number-theory/basic.md#%E5%BF%AB%E9%80%9F%E4%B9%98) 来进行模数范围在 `long long` 时的乘法操作。


### 高精度快速幂


前置技能：[大整数乘法](./bignum.md#%E4%B9%98%E6%B3%95)
...
- [UVa 1230 - MODEX](http://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&category=24&page=show_problem&problem=3671)<sup>[[存档](https://web.archive.org/web/20220707205931/http://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&category=24&page=show_problem&problem=3671)]</sup>
- [UVa 374 - Big Mod](http://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&category=24&page=show_problem&problem=310)<sup>[[存档](https://web.archive.org/web/20220707205931/http://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&category=24&page=show_problem&problem=310)]</sup>
- [UVa 11029 - Leading and Trailing](https://uva.onlinejudge.org/index.php?option=onlinejudge&page=show_problem&problem=1970)
- [Codeforces - Parking Lot](http://codeforces.com/problemset/problem/630/I)<sup>[[存档](https://web.archive.org/web/20210922185521/http://codeforces.com/problemset/problem/630/I)]</sup>
- [SPOJ - The last digit](http://www.spoj.com/problems/LASTDIG/)<sup>[[存档](https://web.archive.org/web/20230803232040/http://www.spoj.com/problems/LASTDIG/)]</sup>
- [SPOJ - Locker](http://www.spoj.com/problems/LOCKER/)<sup>[[存档](https://web.archive.org/web/20241214040009/http://www.spoj.com/problems/LOCKER/)]</sup>
- [SPOJ - Just add it](http://www.spoj.com/problems/ZSUM/)<sup>[[存档](https://web.archive.org/web/20231211212205/http://www.spoj.com/problems/ZSUM/)]</sup>

**本页面部分内容译自博文 [Бинарное возведение в степень](http://e-maxx.ru/algo/binary_pow)<sup>[[存档](https://web.archive.org/web/20250618063904/http://e-maxx.ru/algo/binary_pow)]</sup> 与其英文翻译版 [Binary Exponentiation](https://cp-algorithms.com/algebra/binary-exp.html)。其中俄文版版权协议为 Public Domain + Leave a Link；英文版版权协议为 CC-BY-SA 4.0。**
