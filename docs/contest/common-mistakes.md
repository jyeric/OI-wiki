...
- 未初始化局部变量。
  ...
  ???+ note "未初始化变量会发生什么"
  原文：<https://loj.ac/d/3679><sup>[[存档](https://web.archive.org/web/20220926073527/https://loj.ac/d/3679)]</sup> by @hly1204
...
- 在类或结构体的构造函数中使用 `:` 初始化变量时，变量声明顺序不符合初始化时候的依赖关系。
  ...
  - 成员变量的初始化顺序与它们在类中声明的顺序有关，而与初始化列表中的顺序无关。参见：[构造函数与成员初始化器列表](https://zh.cppreference.com/w/cpp/language/constructor)<sup>[[存档](https://web.archive.org/web/20241231195522/https://zh.cppreference.com/w/cpp/language/constructor)]</sup> 的「初始化顺序」
...
???+ warning
    ...
    本节内容仅适用于模拟赛等情况，而我们也建议出题人尽量让数据符合 [数据格式](problemsetting.md#%E6%95%B0%E6%8D%AE%E7%9A%84%E6%A0%BC%E5%BC%8F)。
...
- 尝试修改字符串字面量
  ...
  ???+ warning "示例"
  `cpp     char *p = "OI-wiki";     p[0] = 'o';     p[1] = 'i';     `
...
- 使用未初始化的变量
  ...
  ???+ warning "示例"
  `cpp     int foo(int a) {       int t; /* 没有初始化 */       if (/* 使用 */ t > 3) return a;       return 0;     }     `
...
- 使用 + 运算符向 `std::string` 类字符串追加字符
  ...
  从 [汇编结果](https://godbolt.org/z/Eo9vn7or5)<sup>[[存档](https://web.archive.org/web/20240910214556/https://godbolt.org/z/Eo9vn7or5)]</sup> 可以看出，`a = a + b` 调用了三次 `std::__cxx11::basic_string` 中的功能，分别为 `operator+`、`operator=` 和创建变量。
  ...
  [这种写法](https://godbolt.org/z/eGh33Grf3)<sup>[[存档](https://web.archive.org/web/20250519085842/https://godbolt.org/z/eGh33Grf3)]</sup> 会直接将字符 `b` 附加到字符串 `a` 中，仅调用了一次 `operator+=`。更详细的性能比较可参考 [Benchmark](https://quick-bench.com/q/JNDGl7HgOszNG-bo7AgVc42owv4)<sup>[[存档](https://web.archive.org/web/20240417225826/https://quick-bench.com/q/JNDGl7HgOszNG-bo7AgVc42owv4)]</sup>。
