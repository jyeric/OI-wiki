...
使用 [`std::ios::sync_with_stdio(false)`](https://en.cppreference.com/w/cpp/io/ios_base/sync_with_stdio)<sup>[[存档](https://web.archive.org/web/20231126223636/https://en.cppreference.com/w/cpp/io/ios_base/sync_with_stdio)]</sup> 函数来关闭与 C 流的同步。C++ 为了兼容 C，也就是为了保证程序在同时使用了 `printf` 和 `std::cout` 时不发生混乱，因此对这两种流进行了同步。同步的 C++ 流保证是线程安全的。
...
使用 [`tie()`](https://en.cppreference.com/w/cpp/io/basic_ios/tie)<sup>[[存档](https://web.archive.org/web/20220927173709/https://en.cppreference.com/w/cpp/io/basic_ios/tie)]</sup> 函数解除输入流与输出流的关联。
...
???+ note "注意"
    ...
    本页面中介绍的读入和输出优化均针对整型数据。浮点数的读入与输出优化十分复杂，读入相关优化可参考 [Bellerophon 算法](https://dl.acm.org/doi/10.1145/93542.93557)<sup>[[存档](https://web.archive.org/web/20250416182607/https://dl.acm.org/doi/10.1145/93542.93557)]</sup>，输出相关优化可参考 [Ryū 算法](https://dl.acm.org/doi/10.1145/3192366.3192369)<sup>[[存档](https://web.archive.org/web/20250815110125/https://dl.acm.org/doi/10.1145/3192366.3192369)]</sup>。

### 实现设计

???+ note "注意"

    当前的优化方法着重于进行更快的 I/O，而在数据转换过程中均采用朴素方法，并未充分利用硬件特性。现如今绝大多数 x86 架构 CPU 均支持 AVX2 指令集，可以利用 SIMD 加速整数与字符串之间的转换。标准库函数并未利用 SIMD 优化，如 libstdc++ 的 [实现](https://github.com/gcc-mirror/gcc/blob/releases/gcc-14.3.0/libstdc%2B%2B-v3/include/bits/charconv.h#L81)<sup>[[存档](https://web.archive.org/web/20250907193939/https://github.com/gcc-mirror/gcc/blob/releases/gcc-14.3.0/libstdc%2B%2B-v3/include/bits/charconv.h#L81)]</sup> 为一次转化连续两位，并通过查表的方式转化为字符，因此优化数据转换过程也可能会带来收益。但在竞赛范畴，本文中提到的优化方法已足够应对绝大多数场景。
...
在读入数字部分的过程中，需要判断读入的字符是否为十进制数字字符。可简单采用 `ch >= '0' && ch <= '9'` 条件进行判断，也可利用 [`isdigit()`](https://en.cppreference.com/w/cpp/string/byte/isdigit)<sup>[[存档](https://web.archive.org/web/20241223124213/https://en.cppreference.com/w/cpp/string/byte/isdigit)]</sup> 函数。
...
如果程序中使用了多个类型的整型变量，那么可能需要实现多个类型不同但逻辑相同的输入输出函数。此时可以使用 C++ 中的 [`template`](https://en.cppreference.com/w/cpp/language/templates.html)<sup>[[存档](https://web.archive.org/web/20250904222042/https://en.cppreference.com/w/cpp/language/templates.html)]</sup> 实现对于所有整数类型的输入输出优化。例如在 C++11 标准下使用
...
[cin.tie 与 sync_with_stdio 加速输入输出 - 码农场](https://www.hankcs.com/program/cpp/cin-tie-with-sync_with_stdio-acceleration-input-and-output.html)<sup>[[存档](https://web.archive.org/web/20200927013335/https://www.hankcs.com/program/cpp/cin-tie-with-sync_with_stdio-acceleration-input-and-output.html)]</sup>


[C++ 高速化 - Heavy Watal](https://heavywatal.github.io/cxx/speed.html)<sup>[[存档](https://web.archive.org/web/20220924232454/https://heavywatal.github.io/cxx/speed.html)]</sup>


['Re: mmap/mlock performance versus read' - MARC](https://marc.info/?l=linux-kernel&m=95496636207616&w=2)<sup>[[存档](https://web.archive.org/web/20241011002034/https://marc.info/?l=linux-kernel&m=95496636207616&w=2)]</sup>
