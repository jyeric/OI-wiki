...
由于 $h^*(x)$ 的实际值在搜索的时候是未知的，所以，需要使用容易计算的 $h(x)$ 作为它的估计。A\* 搜索的实际复杂度就取决于这一估计函数 $h(x)$ 的性质。容易想象，如果 $h\equiv h^*$，也就是说，估计是精确的，那么，搜索过程就会严格按照最短路径前进。而如果 $h\equiv 0$，那么，A\* 算法就退化为 [Dijkstra 算法](./../graph/shortest-path.md#dijkstra-%E7%AE%97%E6%B3%95)；当 $h\equiv 0$ 并且边权为 $1$ 时，这就是 [BFS](./bfs.md)。
...
- [A* search algorithm - Wikipedia](https://en.wikipedia.org/wiki/A*_search_algorithm)<sup>[[存档](https://web.archive.org/web/20241230005030/https://en.wikipedia.org/wiki/A*_search_algorithm)]</sup>

[^note1]: 此处的 $h$ 意为 heuristic。详见 [启发式搜索 - 维基百科](https://zh.wikipedia.org/wiki/%E5%90%AF%E5%8F%91%E5%BC%8F%E6%90%9C%E7%B4%A2)<sup>[[存档](https://web.archive.org/web/20250605053814/https://zh.wikipedia.org/wiki/%E5%90%AF%E5%8F%91%E5%BC%8F%E6%90%9C%E7%B4%A2)]</sup> 和 [A* search algorithm - Wikipedia](https://en.wikipedia.org/wiki/A*_search_algorithm#Bounded_relaxation)<sup>[[存档](https://web.archive.org/web/20250912021617/https://en.wikipedia.org/wiki/A*_search_algorithm#Bounded_relaxation)]</sup> 的 Bounded relaxation 一节。
