...
> 另请参阅 [常量表达式 constexpr（C++11）](const.md#%E5%B8%B8%E9%87%8F%E8%A1%A8%E8%BE%BE%E5%BC%8F-constexprc11)
...
[元组](https://zh.cppreference.com/w/cpp/utility/tuple)<sup>[[存档](https://web.archive.org/web/20240731103603/https://zh.cppreference.com/w/cpp/utility/tuple)]</sup> 定义于头文件 `<tuple>`，是 `std::pair` 的推广，可以存储多个不同类型的值。下面来看一个例子：
...
它不是一种语言特性，而是一种 [概念或者要求](https://zh.cppreference.com/w/cpp/named_req/FunctionObject)<sup>[[存档](https://web.archive.org/web/20240922003936/https://zh.cppreference.com/w/cpp/named_req/FunctionObject)]</sup>，在标准库中广泛应用。
...
???+ warning "请注意性能开销"
    ...
    `std::function` 会引入一定的性能开销，经 [Benchmark](./lambda.md#lambda-%E4%B8%AD%E7%9A%84%E9%80%92%E5%BD%92) 测试，通常会造成 2 到 3 倍以上的性能损失。
    ...
    请考虑使用 [**Lambda 表达式**](./lambda.md) 或者 [**函数对象**](#%E5%87%BD%E6%95%B0%E5%AF%B9%E8%B1%A1) 代替。

`std::function` 是通用函数封装器，定义于头文件 `<functional>`。


`std::function` 的实例能存储、复制及调用任何 [**可调用**](https://zh.cppreference.com/w/cpp/named_req/Callable)<sup>[[存档](https://web.archive.org/web/20230326082237/https://zh.cppreference.com/w/cpp/named_req/Callable)]</sup> 对象，这包括 [**Lambda 表达式**](./lambda.md)、成员函数指针或其他 [**函数对象**](#%E5%87%BD%E6%95%B0%E5%AF%B9%E8%B1%A1)。


若 `std::function` 不含任何可调用对象（比如默认构造），调用时将抛出 [`std::bad_function_call`](https://zh.cppreference.com/w/cpp/utility/functional/bad_function_call)<sup>[[存档](https://web.archive.org/web/20171102101414/https://zh.cppreference.com/w/cpp/utility/functional/bad_function_call)]</sup> 异常。
...
在需要对容器等范围进行复杂操作时，[范围库](https://zh.cppreference.com/w/cpp/ranges)<sup>[[存档](https://web.archive.org/web/20220615043142/https://zh.cppreference.com/w/cpp/ranges)]</sup> 可以使得算法编写更加容易和清晰。
...
**范围适配器** 作为 [**范围适配器闭包对象**](https://zh.cppreference.com/w/cpp/named_req/RangeAdaptorClosureObject)<sup>[[存档](https://web.archive.org/web/20250811132410/https://zh.cppreference.com/w/cpp/named_req/RangeAdaptorClosureObject)]</sup>，也属于 [**函数对象**](#%E5%87%BD%E6%95%B0%E5%AF%B9%E8%B1%A1)，它们重载了 `operator|`，使得它们能够像管道一样拼装起来。

??? note "管道运算符"

    此处的 `|` 应该理解成管道运算符，而非按位或运算符，这个用法来自于 Linux 中的 [管道](<https://zh.wikipedia.org/wiki/%E7%AE%A1%E9%81%93_(Unix)>)<sup>[[存档](<https://web.archive.org/web/20241202182653/https://zh.wikipedia.org/wiki/%E7%AE%A1%E9%81%93_(Unix)>)]</sup>。
...
这些算法可以理解成旧标准库算法的改良版本，均为函数对象，提供更友好的重载和入参类型检查（基于 [`concept`](https://zh.cppreference.com/w/cpp/language/constraints)<sup>[[存档](https://web.archive.org/web/20220621102545/https://zh.cppreference.com/w/cpp/language/constraints)]</sup>），让我们先以 `std::sort` 和 `ranges::sort` 的对比作为例子
...
1. [C++ 参考手册](https://zh.cppreference.com/)<sup>[[存档](https://web.archive.org/web/20221207045510/https://zh.cppreference.com/)]</sup>
