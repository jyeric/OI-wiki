...
由于 `vector` 重载了 `=` 运算符，所以我们可以方便地进行 `vector` 的整体赋值操作。此外从 C++11 起 `vector` 还支持 [列表初始化](https://zh.cppreference.com/w/cpp/language/list_initialization)<sup>[[存档](https://web.archive.org/web/20250829072741/https://zh.cppreference.com/w/cpp/language/list_initialization)]</sup>，例如 `vector<int> data {1, 2, 3};`。


### `vector` 的使用方法


以下介绍常用用法，详细内容 [请参见 C++ 文档](https://zh.cppreference.com/w/cpp/container/vector)<sup>[[存档](https://web.archive.org/web/20230729122110/https://zh.cppreference.com/w/cpp/container/vector)]</sup>。
...
`std::deque` 是 STL 提供的 [双端队列](../../ds/queue.md#%E5%8F%8C%E7%AB%AF%E9%98%9F%E5%88%97) 数据结构。能够提供线性复杂度的插入和删除，以及常数复杂度的随机访问。


### `deque` 的使用方法


以下介绍常用用法，详细内容 [请参见 C++ 文档](https://zh.cppreference.com/w/cpp/container/deque)<sup>[[存档](https://web.archive.org/web/20250416012538/https://zh.cppreference.com/w/cpp/container/deque)]</sup>。`deque` 的迭代器函数与 `vector` 相同，因此不作详细介绍。
...
`deque` 通常的底层实现是多个不连续的缓冲区，而缓冲区中的内存是连续的。而每个缓冲区还会记录首指针和尾指针，用来标记有效数据的区间。当一个缓冲区填满之后便会在之前或者之后分配新的缓冲区来存储更多的数据。更详细的说明可以参考 [STL 源码剖析——deque 的实现原理和使用方法详解](https://blog.csdn.net/baidu_28312631/article/details/48000123)<sup>[[存档](https://web.archive.org/web/20241231220116/https://blog.csdn.net/baidu_28312631/article/details/48000123)]</sup>。
...
`list` 的使用方法与 `deque` 基本相同，但是增删操作和访问的复杂度不同。详细内容 [请参见 C++ 文档](https://zh.cppreference.com/w/cpp/container/list)<sup>[[存档](https://web.archive.org/web/20250417160005/https://zh.cppreference.com/w/cpp/container/list)]</sup>。`list` 的迭代器、长度、元素增删及修改相关的函数与 `deque` 相同，因此不作详细介绍。
...
`forward_list` 的使用方法与 `list` 几乎一致，但是迭代器只有单向的，因此其具体用法不作详细介绍。详细内容 [请参见 C++ 文档](https://zh.cppreference.com/w/cpp/container/forward_list)<sup>[[存档](https://web.archive.org/web/20250514190329/https://zh.cppreference.com/w/cpp/container/forward_list)]</sup>
