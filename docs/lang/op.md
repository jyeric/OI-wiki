...
对于双目算术运算符，当参与运算的两个变量类型相同时，不发生 [类型转换](./var.md#%E7%B1%BB%E5%9E%8B%E8%BD%AC%E6%8D%A2)，运算结果将会用参与运算的变量的类型容纳，否则会发生类型转换，以使两个变量的类型一致。转换的规则参见 [类型转换](./var.md#%E7%B1%BB%E5%9E%8B%E8%BD%AC%E6%8D%A2)。
...
这些运算符用来访问对象的成员或者内存，除了最后一个运算符外上述运算符都可被重载。与 `&` ， `*` 和 `->` 相关的内容请阅读 [指针](./pointer.md) 和 [引用](./reference.md) 教程。这里还省略了两个很少用到的运算符 `.*` 和 `->*` ，其具体用法可以参见 [C++ 语言手册](https://zh.cppreference.com/w/cpp/language/operator_member_access)<sup>[[存档](https://web.archive.org/web/20221119225003/https://zh.cppreference.com/w/cpp/language/operator_member_access)]</sup> 。
...
来自 [C++ 运算符优先级 - cppreference](https://zh.cppreference.com/w/cpp/language/operator_precedence)<sup>[[存档](https://web.archive.org/web/20221229025118/https://zh.cppreference.com/w/cpp/language/operator_precedence)]</sup> ，有修改。
