...
Lambda 表达式因数学中的 $\lambda$ 演算得名，直接对应于其中的 lambda 抽象。编译器在编译时会根据语法生成一个匿名的 [**函数对象**](./new.md#%E5%87%BD%E6%95%B0%E5%AF%B9%E8%B1%A1)，以捕获的变量作为其成员，参数和函数体用于实现 `operator()` 重载。
...
函数体与普通函数函数体类似，除了能访问参数和全局变量等，还可访问 [捕获](#capture-%E6%8D%95%E8%8E%B7%E5%AD%90%E5%8F%A5) 的变量。
...
详见 [mutable 可变规范](#mutable-%E5%8F%AF%E5%8F%98%E8%A7%84%E8%8C%83)。
...
由于 **parameters 参数列表** 是可选的，如果不将参数传递给 lambda，并且其声明不包含 [mutable](#mutable-%E5%8F%AF%E5%8F%98%E8%A7%84%E8%8C%83)，且没有后置返回值类型，则可以省略空括号。

??? note " 使用 `auto` 声明的参数 "

    **C++14** 后，若参数使用 `auto` 声明类型，那么会构造一个 [泛型 Lambda 表达式](#%E6%B3%9B%E5%9E%8B-lambdac14)。

#### 显式对象形参（C++23）


**C++23** 起，[显式对象形参](https://zh.cppreference.com/w/cpp/language/function#.E5.BD.A2.E5.8F.82.E5.88.97.E8.A1.A8)<sup>[[存档](https://web.archive.org/web/20241227023047/https://zh.cppreference.com/w/cpp/language/function#.E5.BD.A2.E5.8F.82.E5.88.97.E8.A1.A8)]</sup> 可以在 lambda 的参数中使用。
...
在 [cpp insights](https://cppinsights.io)<sup>[[存档](https://web.archive.org/web/20201230225715/https://cppinsights.io)]</sup> 中可以观察到编译器生成的 `lambda` 类定义：
...
使用 [std::find_if](https://zh.cppreference.com/w/cpp/algorithm/find)<sup>[[存档](https://web.archive.org/web/20161102090459/https://zh.cppreference.com/w/cpp/algorithm/find)]</sup> 查找第一个大于 3 的元素：
...
- [cppreference-lambda](https://en.cppreference.com/w/cpp/language/lambda)<sup>[[存档](https://web.archive.org/web/20241226045742/https://en.cppreference.com/w/cpp/language/lambda)]</sup>
